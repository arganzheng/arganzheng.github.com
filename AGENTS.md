...
- `/slides/my-talk.html` — `_layouts/slides.html`, the **landing page**
  (SlideShare style): the deck plays in a 16:9 iframe with a bar (上一页 /
  下一页 / `n / N` / PDF / 新窗口 / 全屏, `js/slides-player.js` drives the
  iframe's `Reveal` directly — same origin), then the post-style header meta
  (author · date · pages · 阅读 / 点赞 / 评论 via `.post-stats`), every slide
  laid out flat as a numbered `section.deck-page#pN` inside a
  `.post-container` (so 划线评论, code copy, line numbers, figure tools all
  work; speaker notes render as a 演讲备注 box), the action bar and the
  comments. Views / likes / the Discussion are keyed on this URL like a post.
  Position in the URL is `#/N` (replaceState); clicking a flat page's number
  plays it. Styles: `less/slides.less`.
