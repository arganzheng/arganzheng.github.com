...
| 地图 | 面向 | 一句话 | 层 | 已写成的系列 |
|---|---|---|---|---|
| [AI 算法工程师](/ai-algorithm-engineer-learning-roadmap.html) | **造模型的人**：能复现论文、设计后训练配方、把模型评测清楚 | 为什么这样建模、效果如何、怎么证明 | 八层 + 横切 | 八个系列 44 篇 + 横切一篇，全部写完；另与 Infra 共享 01、03、04 三个系列 |
...
**开头提问，结尾作答，再考一遍。** 每篇文章开头有一个粗体的核心问题，小结之后有一个折叠的「核心问题的答案」逐条回应它，再往后是一节「自测」——三到五道有确定答案的题，答案各自折叠。先自己答、再展开对，是检验"读懂了"最便宜的办法；答不上来就回到对应的章。算法侧的系列已全部按这个格式整理，Infra 系列陆续跟上。
...
| 地图 | 已写 | 待写 |
|---|---|---|
| AI-Infra | 十一个系列共 93 篇、约 190 小时：Python 7（14h）、C++ 8（33h）、PyTorch 10（15h）、Transformer 与 LLM 8（11h）、GPU kernel 10（21h）、通信 8（20h）、大规模训练 8（22h）、vLLM 14（17h）、[RL 后训练基础设施](/rl-post-training-infrastructure.html) 8（8h）、平台 8（21h）、开源贡献 4（9h） | 两个选修（ML 编译器、扩散模型推理基础设施）暂以地图里的段落代替，后者计划作为短系列补上 |
| AI 算法工程师 | 专属 45 篇、约 30 小时：L0 数学 8（3h），L1 工具箱 5（2h，深入篇共享 Infra 01、03），L2 经典机器学习 6（2h），L3 深度学习基础 6（3h），L4 共享 04 系列 + 预训练 4（4h），L5 后训练 8（6h），L6 高效推理与压缩 6（4h），L7 多模态 7（5h），横切实验方法论 1（1h） | — |
