...
| 后端工程师，想转 AI 但方向未定 | 算法地图 L0 → L1（有编程基础的人第一篇过一遍即可，重点是第二篇起的科学计算栈与 PyTorch 用法），然后按[「全栈的一条线」](#全栈的一条线)的第 2、3 段走到 04 系列 | 到 04 系列读完再定方向：做应用转应用地图，做系统进入 Infra 地图并回头精读 02 / 03，做模型继续第 4 段；深入篇不要一开始就读 |
