...
**benchmark 涨 5 个点**：先问测试集是不是还是测试集。它一旦参与过任何决策（挑配置、挑 checkpoint）就变成了验证集，分数带选择偏差——20 个测试点上挑最好的配置平均乐观 0.255（第五章）；LLM 上另一种形态是污染——测试题在预训练语料里。检查的办法是换一份没参与过决策、确认不在语料里的题重测。**奖励模型 80% 准确率为什么还被钻空子**：RM 是在有限偏好数据上拟合的分类器，它在数据分布外的判断不受约束；RL 在它上面做优化就是专门去找它判断错的地方——在一个过拟合的评估器上做优化，经典机器学习叫"在验证集上选模型"，这里叫 reward hacking（第八章）。80% 是分布内的准确率，说不了分布外的事。
