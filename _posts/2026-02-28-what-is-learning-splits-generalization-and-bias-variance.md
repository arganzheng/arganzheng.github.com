...
| 六 | 偏差-方差 | 分解公式逐项解释；100 个模型的预测带；集成只降方差；LLM 上的三种集成 |
...
两个误差：**训练误差**是 $$f$$ 在训练数据上的平均损失；**泛化误差**是它在全部可能数据上的期望损失。我们能算前者、关心后者。两者的差距叫**泛化差距**——它是本篇一切讨论的对象。本文用的损失是**均方误差**（MSE，mean squared error）：预测值与真值之差的平方，再取平均——差 0.3 的平方是 0.09，所以噪声标准差 0.3 对应的 MSE 是 0.09，这个数字后面会反复出现。
...
`X` 是 `[样本数, 特征数]` 的二维数组，`y` 是 `[样本数]`。整个系列不需要更多接口。
...
1. ① 把每个 $$x$$ 展开成一行 $$[1, x, x^2, \ldots, x^d]$$，30 个点就是一个 $$30 \times (d+1)$$ 的矩阵 $$A$$；
2. ② 求 $$w$$ 使 $$A w$$ 尽量接近 $$y$$——这就是**最小二乘**，第二篇会推导它为什么有闭式解；
3. ③ 预测新点时用同样的展开乘上 $$w$$。
...
表的后两列：把 20 个（各用不同数据训的）模型的预测**平均**，1 次与 4 次的方差降到原来的几十分之一，偏差**不变**（1 次的 0.158 → 0.152）。这叫**集成**（ensemble）；用重采样的数据训多个模型再平均叫 **bagging**，随机森林（第六篇）就是它。原理是 L0 第四篇的"独立量的平均，方差除以 $$n$$"——15 次多项式的方差只降了一半而不是 20 倍，是因为 20 个模型用的数据有重叠、误差不独立。
