...
flowchart TB
    subgraph LOOP["一条 agent 轨迹（重复 20 轮）"]
        direction TB
        L1["推理引擎（GPU）：生成一段，含 tool_call"] --> L2["解析工具调用"] --> L3["沙箱容器（CPU）：跑测试 / 执行命令——几十秒，占着一个有状态的环境"] --> L4["结果追加进上下文"] --> L1
    end
    LOOP --> RW["环境服务：判分（跑测试 / 比对）→ reward → 训练器"]
    N["几千条轨迹同时在跑：GPU 在等沙箱时空着，沙箱在等 GPU 时空着；最慢的那条决定这一步何时结束（第六章）"] -.-> LOOP
