...
> **Adam 的两个矩各在做什么？[^q0] 为什么 AdamW 与在 loss 里加 $$L_2$$ 不一样？[^q1] warmup 为什么在 Adam 下几乎不能省？[^q2] batch 变大时学习率该怎么变、变到哪里为止？[^q3]**
...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | SGD 与它的噪声 | 无偏估计、方差与 1/B、线性 scaling 规则的推导与失效点；实测到 B=512 成立、2048 发散 |
| 三 | Momentum | 指数移动平均、有效学习率放大 1/(1−β)、Nesterov |
| 四 | Adam | 两个矩、偏差修正、第一步的更新量恰好是 η、β₂ 的记忆窗、ε、每参数 8 字节 |
| 五 | AdamW 与 L2 的区别 | L2 被 1/√v 缩放、AdamW 不被；实测同一 λ 下权重范数 2.3 vs 23.6；LLM 里 weight decay 的平衡点含义 |
| 六 | 学习率调度 | warmup 为什么必须、cosine 与 WSD、衰减到多少、峰值学习率的量级与宽度的关系 |
...
**有偏差修正时，第一步每个参数都恰好移动 $$\eta$$**，与梯度大小完全无关。第九章实测最大 $$\lvert \text{更新量} \rvert / \eta = 1.00$$。没有偏差修正时是 $$(1 - \beta_1) / \sqrt{1 - \beta_2}$$：$$\beta_2 = 0.999$$ 下是 3.16（实测 3.16），$$\beta_2 = 0.95$$ 下是 0.45。
...
[^q0]: $$m$$ 是梯度的指数移动平均（Momentum，平滑方向），$$v$$ 是梯度平方的指数移动平均（每个参数的尺度）；更新量 $$\hat m / \sqrt{\hat v}$$ 是量级约 1 的无量纲数，所以每个参数每步走约 $$\eta$$、与梯度大小无关——一个学习率适用于尺度相差几个量级的所有参数。详见[第三章](#三momentum)、[第四章](#四adam)。
[^q1]: $$L_2$$ 的 $$\lambda\theta$$ 混进 $$g$$ 后被 $$1/\sqrt{v}$$ 缩放，梯度小的参数被过度衰减（实测 $$\lVert W_1 \rVert$$ 从 22.6 掉到 2.3，准确率 95.6% → 86.2%）；AdamW 把衰减放在矩之外，均匀作用于所有参数。SGD 下两者等价，Adam 下不等价。详见[第五章](#五adamw-与-l2-正则的区别)。
[^q2]: 偏差修正后 Adam 第一步的更新恰好是 $$\eta \cdot \text{sign}(g)$$——满步长——此时 $$v$$ 还没学到真实尺度，所有参数同时以最大步长乱跳；$$\eta = 10^{-2}$$ 无 warmup 时 loss 冲到 4.59、有则 1.26。详见[第六章](#六学习率调度)。
[^q3]: SGD 下梯度噪声方差 $$\propto 1/B$$，$$B$$ 乘 $$k$$ 则 $$\eta$$ 乘 $$k$$（线性 scaling），Adam 下近似 $$\sqrt{k}$$；在临界 batch 之内成立（实测到 512），之外发散（2048）——超过临界 batch 再加 batch 只是浪费样本。详见[第二章](#二sgd-与它的噪声)、[第六章](#六学习率调度)。
