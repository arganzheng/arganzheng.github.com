...
flowchart TB
    subgraph STEP["一个去噪步（FLUX，无 CFG）"]
        direction TB
        IN["x_t, t, c"] --> EMB["时间步 embedding + adaLN 调制向量
（1 个 token 的 MLP，忽略）"]
        EMB --> B1["block 1：attention + MLP（4 个 GEMM + 1 个 attention + 十几个小算子）"]
        B1 --> B2["block 2 … block 57：同上，共 57 层"]
        B2 --> OUT["预测 ε / v：[N, c·p²]"]
        OUT --> SOLVER["采样器（逐元素，忽略）"]
        SOLVER --> NEXT["x_{t-1}"]
    end
    NEXT -. "× 28" .-> IN
...
Table: 主流扩散模型的 latent 与 token 数
...
- **视频模型的 attention 占 70–90%**。Wan 一步 6.5 PFLOPs 里 4.7 P 是 attention；HunyuanVideo 129 帧的 attention 是线性项的 6.5 倍。按 $$4 L N^2 d$$ 算：$$4 \times 60 \times 119{,}056^2 \times 3072 = 10.5$$ P，一步 12.1 P，50 步 604 P（算法地图 L7 第六篇的成本一节给出的就是这笔账的粗略版本）。视频生成的 attention 比直觉更重——早期的粗估常把它当作"与线性项相当"，差了一个 6 倍。
...
flowchart TB
    subgraph ROOF["H100 roofline 上的位置（算术强度从低到高）"]
        direction TB
        A["LLM decode，batch 1：强度 2
带宽屋顶：读 14 GB 权重 = 4 ms / token"] --- K["拐点 295 FLOP/字节（989 T / 3.35 TB/s）"]
        K --- B["DiT 一次前向，batch 1：强度 3,100（FLUX）
算力屋顶：74 T / 989 T = 75 ms（100% MFU）"]
        B --- C["视频 DiT 一次前向：强度 240,000（Wan）
attention 主导，更深地在算力屋顶上"]
    end
...
Table: memory-bound 与 compute-bound 的对照
...
Table: 7B LLM 与 FLUX.1-dev 的负载对照
...
Table: FLUX 1024² 在不同步数下的 DiT 段时间
...
Table: CFG ×2 与 guidance 蒸馏 ×1 的时间
