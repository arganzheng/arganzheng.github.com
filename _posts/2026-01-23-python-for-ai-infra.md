...
第一篇沿着"一段代码的生命周期"这条线组织：执行模型与执行帧、作用域与闭包、模块与导入系统、类与对象模型（属性查找算法与描述符）、对象协议、装饰器、生成器、上下文管理器、异常处理，最后用一个推理组件把它们串起来完整追踪一遍。
...
之后按三种执行模型分章展开——线程（Java 工程师最熟悉的模型）、进程（绕开 GIL 的代价）、asyncio（单线程内的 M:N 调度）——每一章按同样的顺序讲：怎么启动任务、任务之间怎么协作（Future、有界队列与背压、同步原语、上下文传递）、这种模型的边界在哪（线程不能取消、进程只能 kill、协程可以协作式取消）。同一个概念会出现三次，后两次只讲差别。最后用批处理、流式、GPU 混合并发这些 AI-Infra 组合模式把三种模型拼在一起。
...
**第一遍怎么读（全栈 / 新手读者）**：1（只读第五章对象模型、第七章装饰器、第八章生成器）→ 3（第四章 asyncio 与第七章并发决策树）→ 6（工具与决策树）。算法地图 L1 工具箱假设你会写 Python；这三处是读训练代码与推理框架源码时最先卡住的地方，约 4 小时。其余在需要时回来。
...
**配套代码**：写文章时用来验证各个结论的小脚本（目前是第一篇的语言机制与第三篇的并发部分）在 [ai-learning-labs/python-for-ai-infra](https://github.com/arganzheng/ai-learning-labs/tree/main/python-for-ai-infra)，只依赖标准库，`python3 xxx.py` 直接运行。
