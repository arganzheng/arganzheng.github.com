...
![SVD 的几何：单位圆经 Vᵀ 旋转、Σ 拉伸、U 旋转变成椭圆](/img/in-post/orthogonal-rotation-svd-and-low-rank-svd-geometry.svg)

看图就明白"奇异值"是什么：把单位圆上所有的向量都送进 $$W$$，出来的是一个椭圆；椭圆最长的半轴是 $$\sigma_1$$、最短的是 $$\sigma_n$$，长轴的方向是 $$u_1$$，而输入里被拉成长轴的那个方向是 $$v_1$$。一个矩阵"主要在干什么"，就是把 $$v_1$$ 方向的分量放大 $$\sigma_1$$ 倍送到 $$u_1$$ 方向去——这句话是下一章低秩近似的全部依据。
