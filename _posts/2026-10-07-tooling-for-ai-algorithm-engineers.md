...
```text
第二章   Python                会读会写训练代码需要的语言特性；不需要的部分
第三章   科学计算栈            NumPy 的形状与广播、einsum 作为读公式的工具、Pandas 做错误分析、Matplotlib 看曲线
第四章   PyTorch 使用层        五个核心对象与一个最小训练循环；混合精度；显存的账：全量微调 vs LoRA vs QLoRA；多卡启用
第五章   Hugging Face 生态     六个库各管什么；一次微调的组装；为什么读源码是最快的路
第六章   GPU 直觉              两个上限与 roofline 直觉、显存四块、OOM 归因、读 profiler
第七章   实验工具              跟踪、配置、随机性、可复现的最小记录
第八章   自测清单              五件做出来的事
第九章   怎么学                材料与顺序
第十章   本文小结
```
