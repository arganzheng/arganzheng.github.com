...
**能。** 两条规则就够：形状规则 $$[m, k] \times [k, n] \to [m, n]$$（内维必须相同，第三章），成本规则 $$2mnk$$ FLOPs（第四章）。代入 Llama-3-8B：一个 token 过一个 $$4096 \times 4096$$ 的 $$W_Q$$ 是 $$2 \times 4096^2 \approx 33.5$$ M FLOPs；一层七个矩阵 218 M 参数、436 M FLOPs；全模型 8.03 B 参数、一个 token 前向约 15 G FLOPs $$\approx 2N$$（第六章）。批维度括起来只看最后两维，逐元素运算比矩阵乘少一个 $$k$$ 因子所以算账时只数矩阵乘。
