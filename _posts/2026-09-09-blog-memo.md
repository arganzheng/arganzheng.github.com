...
#### V2.23 — 2026-09-15：地图加时长与路线图，系列文章统一「核心问题的答案」与折叠自测
...
- 系列文章收尾结构统一为：小结 → 折叠的「核心问题的答案」（回应文首粗体问题）→ 自测（3–5 题，每题答案各自折叠）→ 下一篇。折叠用 `<details markdown="1">`，样式见 `less/extras.less` 末尾（手工追加到两份 CSS）。先铺算法侧，Infra 系列看反馈再定。
