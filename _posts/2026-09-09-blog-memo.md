...
- **投票 / 点赞**：评论的 ▲ 分数 ▼ 是 GitHub 的 👍/👎 reaction；文章的「赞同 / 反对」是讨论串本身的 👍/👎。一人一票，GitHub 保证。评论区顶部的「最受关注的段落」按投票和讨论数排名。
...
知乎式的一栏（`_includes/post-actions.html` + `js/share.js`）：「▲ 赞同 N ▼ · N 次阅读 · N 条评论 · 分享」。文章页放在 COMMENTS 标题上方；首页和 `/life/` 的每个条目下面是紧凑版（`compact=true`）。数据来源：文章页由 `js/annotations.js` 填（它本来就持有讨论串状态），列表页由 `share.js` 一次请求 Worker 的 `GET /stats?paths=…`（每篇的阅读数 + giscus 公开 API 里的评论数 / 👍 / 👎，边缘缓存 2 分钟）。赞同 / 反对是讨论串上的 👍 / 👎 reaction，列表页也能直接投（登录后跳回列表页；没人评论过的文章会先建讨论串）。
...
- 知乎式操作栏「赞同 / 反对 · 阅读 · 评论 · 分享」：文章页在 COMMENTS 上方（原评论区头部的「有用」并入，改成 👍/👎 双向），首页与 `/life/` 每个条目下有紧凑版，数据走 Worker 新增的 `GET /stats`。
...
- 文章「有用」= 讨论串的 👍；评论 ▲ 分数 ▼ = 👍 / 👎 reaction，一人一票；阅读数 = Worker + Cloudflare D1。
- 用户可见文案统一为「评论 / 划线评论」。
