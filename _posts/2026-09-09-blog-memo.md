...
- 文章页在作者通过 GitHub 登录后，在「复制为公众号格式」旁显示「编辑文章」；入口直接跳转 GitHub 对应的源码编辑页面。`header-post` 与 `keynote` 也使用这个操作栏，`slides` 页面在右下角提供「编辑幻灯片」入口。所有入口都不在渲染后的 HTML 与 Markdown 之间转换，也不把仓库写权限交给博客 Worker。
