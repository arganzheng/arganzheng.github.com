...
featured-condition-size: 1                # 文章数 > 1 的标签才上榜
...
**构建失败，日志里有 `Liquid Exception`？** 九成是代码块里的 `{% raw %}{%{% endraw %}` / `{% raw %}{{{% endraw %}` 没包 raw（第四节第 8 小节），或 front matter 的 YAML 写错（冒号后没空格、引号没闭合）。
