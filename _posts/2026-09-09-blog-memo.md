...
- **作者仪表盘** [/admin/stats.html](/admin/stats.html)（不在导航、不进 sitemap、noindex）：阅读趋势（`/views/daily`，7 / 30 / 90 天柱状图 + 这段时间读得最多）、文章榜（`/stats/top`：阅读 · 有用 · 有用率 · 分享 · 评论，点表头排序，默认 TOP 10 可展开）、读者划出来的句子（`/reactions/top`：存疑最多 / 点赞最多 / 分享最多，点句子直达原文）、最近评论（用评论框同一个 GitHub 登录读 Discussions）、待处理的勘误 Issue 与死链报告。数据全是公开的，只是集中在一页。
