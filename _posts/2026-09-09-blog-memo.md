...
#### V2.27 — 2026-09-15：文章页作者 Markdown 编辑器
- 普通 `post` 文章页在作者通过 GitHub 登录后显示「编辑文章」入口：左侧编辑原始 Markdown，右侧保留当前渲染页面作为参考；不把 HTML 反转成 Markdown，避免破坏 front matter、代码、表格、脚注和 Jekyll 扩展。
- 保存通过 `tools/annotations-worker` 的作者专用 `/article` 路由直接提交 `master`，用 GitHub viewer 校验作者、GitHub App Contents 权限和 blob SHA 防止覆盖并发修改。保存会触发 GitHub Pages 构建；Worker 需要 App 的 `Contents: Read and write` 权限。
