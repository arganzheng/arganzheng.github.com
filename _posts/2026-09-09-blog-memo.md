...
- **投票 / 点赞**：评论的 ▲ 分数 ▼ 是 GitHub 的 👍/👎 reaction，一人一票由 GitHub 保证；文章的「赞同 / 反对」是匿名的——Worker 的 D1 里一行计数，不用登录，一个浏览器一票（localStorage 记着），信任级别和阅读数一样。评论区顶部的「最受关注的段落」按评论投票和讨论数排名。
...
### 4. 操作栏（赞同 / 阅读 / 评论 / 分享）与「复制为公众号格式」

知乎式的一栏（`_includes/post-actions.html` + `js/share.js`）：「▲ 赞同 N ▼ · N 次阅读 · N 条评论 · 分享」。文章页放在 COMMENTS 标题上方；首页和 `/life/` 的每个条目下面是紧凑版（`compact=true`）。数据来源：赞同 / 反对是 Worker D1 里的匿名计数（`GET/POST /votes`，不用登录，一个浏览器一票），阅读数同样在 D1；评论数来自 giscus 公开 API。文章页的阅读数和评论数由 `js/annotations.js` 填，列表页由 `share.js` 一次请求 `GET /stats?paths=…` 拿到全部（每篇边缘缓存 2 分钟）。
...
- 知乎式操作栏「赞同 / 反对 · 阅读 · 评论 · 分享」：文章页在 COMMENTS 上方，首页与 `/life/` 每个条目下有紧凑版。文章的赞同 / 反对改为匿名计数（Worker `/votes`，D1），不再是需要 GitHub 登录的讨论串 👍；列表页数据走 Worker 新增的 `GET /stats`。
...
- 文章「赞同 / 反对」= Worker + D1 匿名计数；评论 ▲ 分数 ▼ = GitHub 👍 / 👎 reaction，一人一票；阅读数 = Worker + Cloudflare D1。
- 用户可见文案统一为「评论 / 划线评论」。
