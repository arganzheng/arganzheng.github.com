...
**代码引用**：把下面的一段说明和代码块里的某几行绑起来。代码里在要指的那行**上面**单独写一行注释 `# !ref 名字`（`+N` 再多包 N 行；`//`、`--`、`;`、`/* */`、`<!-- -->` 等注释符都认），正文用普通链接 `[文字](#名字)` 指过去。看效果——悬停下面的划线文字，或悬停代码左侧蓝色的行号：
...
源码长这样（指令行在渲染时被**删掉**：不占行号、复制不带、搜索不索引；下面这个块前面写了 `{:.no-refs}`，指令才得以原样显示）：
...
规则：有 `!ref` 的块自动带行号（标记就是那个蓝色行号）；悬停正文引用 → 对应行亮起，点击 → 不在视口时滚过去并闪一下（没有 JS 就是普通锚点跳转）；悬停 / 轻触蓝色行号 → 就地弹出那段说明。名字全文唯一（同一段代码的多语言 tab 各写一份同名是允许的，锚点落在第一个）；`!ref` 了却没人引用，构建时 warn；正文引用了不存在的名字，`npm run check` 的链接检查会红。指令只认独占一行的写法，写在代码行尾的不算。
