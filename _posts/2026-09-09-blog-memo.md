...
- **作者仪表盘** [/admin/stats.html](/admin/stats.html)（不在导航、不进 sitemap、noindex）：阅读排行（Worker `/views/top`，可切「最近有人读」）、最近评论（用评论框同一个 GitHub 登录读 Discussions）、待处理的勘误 Issue 与死链报告。数据全是公开的，只是集中在一页。
...
- **投票 / 点赞**：评论的 ▲ 分数 ▼ 是 GitHub 的 👍/👎 reaction，一人一票由 GitHub 保证；文章的「有用」是匿名的——Worker 的 D1 里一行计数，不用登录，一个浏览器一票（localStorage 记着），信任级别和阅读数一样。评论区顶部的「最受关注的段落」按评论投票和讨论数排名。
