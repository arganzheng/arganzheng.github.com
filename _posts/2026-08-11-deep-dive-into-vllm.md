...
前五篇——问题定义、指标、请求生命周期、调度、KV cache——约 5 小时，建立推理系统的整体视角；应用地图的读者只读前两篇，就能理解 token 计费与延迟形态（TTFT / TPOT）从哪来。GPU 执行、解码扩展、多卡、模型适配、请求形态、硬件抽象、PD 分离、演进、源码走读九篇在做推理系统时读。
