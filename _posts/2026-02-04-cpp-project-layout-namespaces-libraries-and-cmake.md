...
匿名命名空间（[上篇第四章](/cpp-compilation-model-and-project-layout.html#四one-definition-rule同一个名字只能有一个定义)）是 Java 完全没有的：它的目的不是组织名字，而是控制链接属性。
