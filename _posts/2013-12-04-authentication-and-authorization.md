...
由于非对称加密算法的计算复杂度相比对称加密高3个数量级，不太可能用作通信数据的加密。所以验证通过之后，服务端和客户端利用ECDH交换密钥算法独自计算出一样的密钥，后面的通讯应该还是基于对称加密。


加上非对称加密之后，即使黑客知道了S1，也没有办法伪造服务器，因为他不知道serverPrivateKey。黑客也不可能伪造中间人。因为即使拖库知道S1，黑客如果不通过窃听网络得到A1，用S1解密A1从而得到A1中的H1的情况下，基本是不可能通过S1反推H1的。现在A1包在不对称加密算法上，黑客如果不知道serverPrivateKey和clientPrivateKey就无法解开会话信息。但是，拖库情况下，并且黑客知道serverPublicKey，那么黑客是可以伪造客户端的。为了避免这个情况，最简单的做法是在客户端HARDCODE公钥，不要尝试推送公钥，服务器升级私钥也强制客户端升级。乍看很二，其实很强大。当然，通过反编译客户端程序还是有可能得到serverPublicKey的，不过相对来说增加了难度。客户端自己也生成了公钥和私钥，目的是为了验证服务器是真正的服务器，这个得以成立的前提是公钥是真正的服务器公钥，如果是伪造服务器的公钥，那等于向黑客公开了自己加密的内容，黑客就知道了客户端的公钥，就能伪造服务器的返回数据。


ECDH的安全性依赖于私钥的保密性，如果私钥被盗，上述通过ECDH解决的问题仍然存在。ECDH的私钥保密有以下一些办法（考虑到实现成本，我们推荐2
