...
**FSQ**（Finite Scalar Quantization，Mentzer 等 2023）：把 $$z_e$$ 投影到很低的维度 $$d'$$（比如 5），每一维独立地 round 到 $$L$$ 个等间隔的值（比如 $$L = 8$$：先用 $$\tanh$$ 压到 $$[-1, 1]$$，再乘 $$(L-1)/2$$ 平移到 $$L$$ 个整数刻度上 round——直接对 $$[-1, 1]$$ round 只有 3 级），码本就是这个 $$d'$$ 维格点的**隐式**乘积——$$L^{d'} = 8^5 = 32768$$ 个码字，不需要学，没有"死码字得不到梯度更新"这类学习型码本的坍缩机制。但"利用率 100%"不是保证——encoder 输出若集中在少数格点，用到的码字一样少（toy 里 256 个隐式码字用到 245 个，是这份数据的结果）；FSQ 去掉的是码本学习的病，不是分布集中的病。用 toy 的 4 维 patch 手算（$$L = 4$$，刻度 $$\{0, 1/3, 2/3, 1\}$$）：第 0 张图第 5 个 patch 的像素 $$(0.94, 0.12, 0.75, 0)$$ 逐维 round 到 $$(1, 0, 2/3, 0)$$，刻度编号 $$(3, 0, 2, 0)$$ 按 4 进制拼成一个整数 $$3 + 0 \times 4 + 2 \times 16 + 0 \times 64 = 35$$——这就是它的 token；隐式码本 $$4^4 = 256$$ 个，1797 张图用到 245 个，重建 MSE 0.0043，比 $$K = 32$$ 的 K-Means 码本还好而根本没有训练。STE 传梯度，没有 commitment 与 codebook loss。效果与 VQ 相当，训练简单得多。
...
$$1024^2$$ 图、$$f = 16$$：4096 个 token，栅格 AR 是 4096 步 decode，每步一次前向（KV cache，memory-bound）。一个 7B AR 模型：每步读 14 GB 权重 + KV，约 20–30 ms → **一张图 100 秒**。这就是栅格 AR 在高分辨率上不实用的原因。VAR：10 步，每步只算**新增**那一个尺度的 token（前面尺度的 KV 缓存着），10 个尺度合计 $$\sum_k n_k = 680$$ 个 token，compute-bound，2B 模型约 $$2 \times 2B \times 680 \approx 2.7$$ TFLOPs（若不缓存、每步重算全部前缀，是 $$\sum$$ 前缀 $$= 1710$$ 个 token 位置、6.8 TFLOPs；"680 × 10 ≈ 27 T"是把两种口径混在一起的算法）、不到 1 秒。注意这张表把 $$256^2$$ 的 VAR / MaskGIT / DiT 与 $$1024^2$$ 的栅格 AR / FLUX 放在一起，只能看量级，不能读成"算法本质快 N 倍"。MaskGIT：12 步 × 全图前向。
...
| VAR 2B，10 尺度（$$256^2$$） | $$2 \times 2B \times 680 \approx 2.7$$ T（KV 缓存；不缓存 6.8 T） | 10 | 算力 | < 1 s |
