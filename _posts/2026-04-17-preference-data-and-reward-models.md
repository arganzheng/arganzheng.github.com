...
人与人对同一对回答的判断一致率有明确的测量：Stiennon 等 2020 的摘要任务上标注者之间 73%、研究者之间 77%；InstructGPT 的标注者之间 72.6 ± 1.5%；Anthropic 报告类似的量级。两个回答越接近，判断越接近随机，一致率就越低。

一个常见的推论是"人都只有 73% 一致，RM 准确率的上限就是 73%"。**这个推论不成立。**一致率是两个都会犯错的人互相比较的结果，不是标签对真值的准确率。一个极简的例子：两个标注者各自独立地有 80% 概率标对，他们之间的一致率是 $$0.8^2 + 0.2^2 = 68\%$$；而一个学到了"真值"的 RM，对其中任何一人的标签仍能有 80% 的准确率，高于 68%。反过来，两人若都被同一个表面特征（长度）带偏，一致率可以很高、标签却系统性地错。所以一致率是**标签噪声的线索**（越低说明任务越含糊、需要更多标注者投票或更细的协议），不是模型能达到的天花板；一致率与 RM 验证准确率经验上同量级，是因为两者都受同一个任务模糊度支配，不是因为前者封住了后者。

它真正的用法是**校准期望**：RM 验证准确率到 70% 附近再上涨时，要看训练/验证 gap 与按难度分桶的表现（第四章、第七章），而不是拿一致率当停止线；标签信息量也不能用 $$1 - H(p)$$ 乘对数来算——那个公式假设真标签均匀、噪声独立且与内容无关，把一致率当误标率代进去没有依据。RM 能从几万对里学到东西，靠的是基座模型里已有的"什么是好回答"的先验——它只需要被校准，不需要被教。
...
三家都训 **1 个 epoch**，InstructGPT 明确说多训就过拟合。原因是数据少、模型大：几万到几十万对偏好，RM 有几十亿参数——它在一遍之内就能把训练对全部"记住"（训练准确率到 90%+），验证准确率停在 70% 左右。lr 比 SFT 更小（$$10^{-5}$$ 到 $$5 \times 10^{-6}$$），warmup 短，通常不做 weight decay。
...
Table: 对抗 reward hacking 的对策与代价
...
2. 人与人的标注一致率 72%，RM 在验证集上准确率 74%——RM 是不是已经过拟合了？
   ...
   单看这两个数判断不了。72% 是人与人互相比较的一致率，不是标签的错误率，也不是 RM 的上限（第二章 §4：两个各 80% 正确的人一致率只有 68%）。判断过拟合要看训练 / 验证准确率的 gap 和按难度分桶的曲线：训练 95%、验证 74% 且验证已不再涨，才是过拟合；训练 78%、验证 74% 则还有余量。
...
[^q0]: 因为它提供的是**梯度方向而不是判决**——RL 在几百个样本上取平均，随机错误被平掉；且它的准确率通常停在 70–75%——与人际一致率同量级，但那是任务模糊度的体现，不是上限（第二章 §4）。详见[第二章](#二偏好数据)、[第四章](#四奖励模型的训练)。
