...
> **一个准确率 70% 的奖励模型为什么够用？[^q0] 它在什么地方会被策略钻空子，怎么在训 RL 之前就发现？[^q1]**
...
1. Bradley-Terry 下 $$r_w - r_l = 2$$，人偏好 $$y_w$$ 的概率是多少？给所有回答的奖励同时加 5，这个概率怎么变？

   <details markdown="1"><summary>答案</summary>

   $$\sigma(2) = 0.88$$；不变——loss 只依赖分差，奖励只定义到每个 prompt 一个常数，这就是 DPO 里 $$Z(x)$$ 能抵消的原因。

   </details>

2. 人与人的标注一致率 72%，RM 在验证集上准确率 74%——RM 是不是已经过拟合了？

   <details markdown="1"><summary>答案</summary>

   接近上限而不是过拟合：标签本身有 28% 噪声，74% 已经把能学的学完了；再训下去准确率只能靠拟合噪声上升。判断过拟合要看训练 / 验证准确率的 gap，不是验证准确率的绝对值。

   </details>

3. best-of-N 采样 $$N = 16$$ 与 $$N = 64$$，策略相对参考的 KL 各约多少 nats？这个数字拿来做什么？

   <details markdown="1"><summary>答案</summary>

   $$\log N - (N-1)/N$$：16 是 1.83，64 是 3.17。在训 RL 之前用 BoN 探测 reward hacking——KL 到这个量级时看 gold 分数（人评）是否已经掉头，就知道 RM 能撑到多远。

   </details>

4. RM 对更长的回答系统性给高分，训 RL 之后会发生什么？为什么“集成三个 RM”修不了它？

   <details markdown="1"><summary>答案</summary>

   策略学会把回答变长（最便宜的涨分方向），gold 质量不升反降——长度是最常见的 hacking。三个 RM 在同一份数据上训出同样的长度偏差，集成只降随机误差、消不掉系统偏差（L2 第一篇）；要用长度归一化或在数据里配平长度。

   </details>

5. 10 万对偏好数据训一个 8B 的 RM 约 7 GPU 小时，为什么说 RM“训便宜、用贵”？

   <details markdown="1"><summary>答案</summary>

   训只过一遍数据；用的时候 RL 每一步要对全部 rollout 打分——GRPO 一步 400 万 token 的 RM 前向约 3 分钟，几千步就是几百 GPU 小时，远超训练它的成本。
...
[^q0]: 因为它提供的是**梯度方向而不是判决**——RL 在几百个样本上取平均，随机错误被平掉；且人自己的一致率也只有 70–75%，RM 到这个数就已经「和人一样准」，再高是在拟合标注者个人。详见[第二章](#二偏好数据)、[第四章](#四奖励模型的训练)。
[^q1]: 在**训练数据分布之外**：RL 把策略推向 RM 给高分但数据里没见过的区域，而 RM 在那里的分数由它学到的捷径（长度、格式、语气）决定。过优化有定量规律——金奖励随 $$\sqrt{\text{KL}}$$ 先升后降，拐点由 RM 的规模与数据决定——所以 RL 有一个 KL 预算。训 RL 之前用 RM 自己的行为暴露它：长度相关、best-of-N 扫描（$$N$$ = 16 相当于 1.8 nats 的 KL，是免费的预演）、对抗探针、校准。详见[第五章](#五reward-hacking)、[第八章](#八动手训一个小-rm-要看的几件事)。
