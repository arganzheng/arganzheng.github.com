...
> **怎么知道一个语料里有什么主题？[^q0] 4096 维的 embedding 怎么"看"？[^q1]**
...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | K-Means | 算法与 15 行实现、$$k$$ 怎么选（肘部、轮廓系数）、它的假设 |
| 三 | DBSCAN | 按密度聚；两个月牙上的对比 |
| 四 | 聚类在数据工程里 | 语料的主题分布、按簇配比、找垃圾簇、SFT 数据的多样性 |
| 五 | PCA | 算法与 6 行实现、解释方差、与 SVD 的关系、低秩直觉 |
...
def kmeans(X, k, n_iter=100, seed=0):
    ...
    centers = X[rng.choice(len(X), k, replace=False)]                     # 初始化：随机挑 k 个点当中心（sklearn 默认用 k-means++ 挑得分散些）
    for _ in range(n_iter):                                               # 重复直到不变：
        d2 = ((X[:, None, :] - centers[None, :, :]) ** 2).sum(-1)           #   [n, k]：每个点到每个中心的距离平方（广播）
        labels = d2.argmin(1)                                               #   分配：每个点归到最近的中心
        new_centers = np.array([X[labels == j].mean(0) for j in range(k)])  #   更新：每个中心移到自己簇内所有点的均值
...
每一步都不会让簇内平方和变大（分配步让每个点到中心更近，更新步里均值是让平方和最小的那个点），所以一定收敛——但收敛到局部最优，结果依赖初始化，实践中跑多次取最好的（`sklearn` 的 `n_init=10`）。在下一节那份 6 簇的数据上，上面这 12 行跑 10 个 seed 取簇内平方和最小的：**7281**，ARI 0.762；`KMeans(6, n_init=10)` 是 7281、ARI 0.761——同一个算法，同一个数。
...
实现比描述短，因为第 3 节会说明"协方差矩阵的特征分解"就是"数据矩阵的 SVD"，直接调 SVD 即可：

```python
def pca(X, k):
    Xc = X - X.mean(0)                                        # 中心化
    U, S, Vt = np.linalg.svd(Xc, full_matrices=False)         # 数据矩阵的 SVD（L0 第三篇）：Vt 的每一行是一个主成分方向
    explained = S ** 2 / (len(X) - 1)                         # 每个主成分的方差 = 奇异值² / (n−1)
    return Xc @ Vt[:k].T, explained / explained.sum()         # 投影到前 k 个方向（[n, k]）；各主成分的解释方差比
```

对下一节的手写数字数据，它算出的前 2 维解释方差 28.5%、前 29 维 95.5%，投影坐标与 `sklearn.decomposition.PCA` 逐元素相差 $$10^{-13}$$（符号可能相反——特征向量乘 −1 仍是特征向量）。
...
配套代码：本文的数字由 [`classical-ml/04_unsupervised.py`](https://github.com/arganzheng/ai-learning-labs/blob/main/classical-ml/04_unsupervised.py) 产生；两段手写实现与它们和 `sklearn` 的对数在 [`04_unsupervised_scratch.py`](https://github.com/arganzheng/ai-learning-labs/blob/main/classical-ml/04_unsupervised_scratch.py)。正文已含全部代码。
...
3. 64 维数据 29 维解释 95% 方差，说明什么？这与 LoRA 有什么关系？
   ...
   数据近似躺在 29 维子空间里，真正的自由度远少于 64；LoRA 假设微调的改动矩阵同样只在少数方向上，$$r = 16$$ 就够。
...
5. 两个 embedding 的余弦相似度 0.85，但这个模型的所有向量对余弦都在 0.8 以上。0.85 说明什么？该怎么处理？
   ...
   几乎没有信息——各向异性让所有对都高；减均值 / 白化后再算，或看它在全部对里的百分位。

   </details>

下一篇讲无监督学习在数据工程里最重要的一个应用：万亿 token 的近似去重——Jaccard、MinHash 的无偏估计、LSH 的 S 曲线，以及"Jaccard 大于 0.7 视为重复"是怎么定出来的。

[^q0]: embedding → K-Means（$$k$$ 用肘部法或轮廓系数）→ 每簇抽几条看。聚类在数据工程里是探索工具，目的是让人能看，不需要它「正确」；看到之后按簇配比、找垃圾簇、SFT 数据保多样性。详见[第二章](#二k-means)、[第四章](#四聚类在数据工程里)。
[^q1]: PCA 先算解释方差比知道有效维度（手写数字 64 维里 29 维解释 95%，数据基本上是低维的），可视化时先 PCA 到 50 维再 UMAP；PCA 前两维只能分开差别最大的类，t-SNE / UMAP 只保局部、不可逆。一个陷阱：embedding 的各向异性让所有对的余弦都高，相似度分数失去含义，减均值或白化之后再比。详见[第五章](#五pca)、[第六章](#六可视化)。
