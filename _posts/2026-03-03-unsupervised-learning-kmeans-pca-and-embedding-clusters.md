...
**语料里有什么主题**：embedding → K-Means（$$k$$ 用肘部法或轮廓系数）→ 每簇抽几条看——聚类在数据工程里是探索工具，目的是让人能看，不需要它"正确"；看到之后按簇配比、找垃圾簇、SFT 数据保多样性（第二、三章）。**4096 维怎么看**：PCA 先算解释方差比知道有效维度（手写数字 64 维里 29 维解释 95%，数据基本上是低维的），可视化时先 PCA 到 50 维再 UMAP；PCA 前两维只能分开差别最大的类，t-SNE / UMAP 只保局部、不可逆（第四、五章）。一个陷阱：embedding 的各向异性让所有对的余弦都高，相似度分数失去含义，减均值或白化之后再比。
