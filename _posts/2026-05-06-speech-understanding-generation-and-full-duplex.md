...
离散声学 token 的 LLM 建模：Moshi 每帧 17 个 token（8 + 8 + 1），主模型每 80 ms 一步（12.5 步/秒），一小时 4.5 万步——但每步是 7B 的一次 decode（约 40 ms），持续占一张卡的一部分。深度 Transformer 每帧生成模型自己的 8 个音频码（用户那 8 个是**输入观测**，不是生成的；HF `modeling_moshi.py` 的 depformer `max_length = num_codebooks + 1`，即 1 个文本前缀 + 8 个输出码），小模型，几 ms。

### 2. 全双工的持续成本

半双工系统在无输入时空闲；全双工模型每 80 ms 跑一步不管有没有人说话。"7B 模型 40 ms/步 in 80 ms"说的是**单路的墙钟占比**，不等于占了半张卡的算力：$$B = 1$$ 的 decode 是 memory-bound，40 ms 里 GPU 的算力几乎空转，同一张卡可以把几十路对话的同一拍 batch 在一起（每路都是实时节拍，但节拍相同正好能对齐），每步时间只略增；真正的限制是显存（每路的 KV cache）与尾延迟。所以正确的说法是：全双工**每路持续消耗**、没有空闲，多路靠 batch 摊薄而不是靠时间片，比半双工（无人说话时零成本）贵，具体贵多少取决于并发数与 KV 预算，不是固定的"半张 H100"。
