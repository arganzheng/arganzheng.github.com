...
> **未来的 Serving 系统，是否会从一个模型执行器，演化为统一管理计算、状态和调度的分布式系统？[^q0] 如果会，vLLM 在其中处于什么位置？[^q1]**
...
Table: 分层 KV Cache 的容量、带宽与适合存放的 KV
...
```mermaid
%% 图：分层 KV Cache 的查找树：本机 HBM → 本机 DRAM / SSD → 远端副本，只有远端有副本时才比较传输与重算的代价
flowchart TB
...
[^q0]: **会，而且已经在发生**。LLM Serving 正在完成四个转变：手工配置 → 自动执行计划（并行度、batch、量化、PD 比例由系统按 workload 与 SLO 推导）；本地缓存 → 分布式状态平面（KV 与 prefix cache 跨实例可发现、可迁移）；单体推理 → 多阶段分布式执行（PD 分离、encoder 池、draft 池、多模型级联）；GPU 利用率 → Goodput、SLO 与成本的联合优化。同时管理可编排的计算、可迁移的状态、可恢复的请求、异构的资源与动态生成的执行计划——这已经是分布式操作系统的职责范围。判断任何「未来方向」是否成立，最后都撞回 KV Cache 这堵墙——它是唯一随时间增长的状态。详见[第二](#二serving-系统正在管理什么)至[七章](#七弹性从故障重试到状态恢复)、[第九章](#九ai-serving-operating-system)。
[^q1]: **执行引擎层**——一台机器（或一组 TP 卡）上把请求变成 token 的那一层，管本地调度、本地 KV、本地硬件抽象；它正在通过 `KVConnector`、PD 分离、外部路由接口、对称内存与 NIXL 集成把边界向外推，但集群级的路由、状态平面、执行计划由 llm-d、Dynamo、Mooncake 一类上层系统承担。类比操作系统：vLLM 是内核里的调度器与内存管理器，上层系统是集群的资源管理器；两层之间的契约（KV 传输、能力发现、指标）是当前最活跃的演进点。详见[第八章](#八vllm-的位置与边界)、[第十章](#十serving-engineer-的角色变化)。
