...
| 时代 | 规则 | 依据 | 代表 |
|---|---|---|---|
| 2020 Kaplan | $$N \propto C^{0.73}$$，模型优先，数据 $$D / N \approx 1.7$$ 就够 | 小模型实验；不数输出层、固定 warmup、超参不随规模调 | GPT-3 175B / 300B token |
...
论文给出的 compute-optimal 结论是 $$N \propto C^{0.73}$$、$$D \propto C^{0.27}$$——也就是算力增加时绝大部分应该花在更大的模型上，数据只需要略微增加；GPT-3 的 175B 参数 / 300B token（$$D / N = 1.7$$）就是按这个规则训的。注意 0.73 **不是**把上面的联合公式代进 $$C = 6ND$$ 求极值得到的——那样算出的指数是 $$\alpha_D/(\alpha_N + \alpha_D) = 0.095/0.171 \approx 0.56$$；Kaplan 的 0.73 来自另一组以训练步数 $$S$$ 为变量、含早停假设的拟合（论文 §5.1–6），两套公式不能拼在一起用。

论文还有一个当时被忽略、后来很重要的观察：**大模型的样本效率更高**——同样的 token 数，大模型的 loss 更低，而且到达同一 loss 用的 token 更少。这一点本身没错，但它导出的"训大模型、别训太久"在两年后被证明主要是实验设计造成的（第 4 节）。
...
| 差异 | Kaplan 的做法 | 影响 |
|---|---|---|
| 输出层的算力怎么数 | $$N$$ 不含 embedding，FLOPs 也没把 lm_head 那一份算全 | 小模型里 embedding / lm_head 占比大（第一篇：0.5B 模型 28%），不数它让小模型看起来比实际"便宜"，斜率被抬高 |
| warmup 长度 | 所有运行用同一个固定的 warmup 步数，不随训练长度变 | 短训练里 warmup 占比过大，小数据量的运行 loss 被系统性抬高，于是偏向多参数 |
| 超参数随规模调 | 所有尺寸共用一组学习率与 batch | 小模型的最优 lr / batch 与大模型不同（第五章 §2），不调让小模型吃亏 |

Table: Kaplan 与 Chinchilla 结论不同的原因

Porian 等 2024（NeurIPS 2024 spotlight）在两个数据集上复现 Kaplan 的实验，把这三个因素逐个修正后得到与 Chinchilla 一致的结果；他们**专门检验了 Hoffmann 等提出的"cosine 衰减长度不匹配是根源"这个假说，结论是否定的**——学习率衰减是否精细对 scaling law 的成立影响很小。所以"Kaplan 错在固定长度的 cosine 调度"这个流传很广的说法要收回：调度不匹配是一个真实的**方法论**问题（下一段），但不是 0.73 与 0.5 分歧的主因。**三个因素里没有一个是"发现了新物理"，全是实验设计。**

与调度相关、也是配套实验里专门处理的一点：**用 cosine 调度训练时，中途的 loss 不能当作 $$L(D)$$ 的点**，因为它包含"学习率还没降下来"的成分。要画 $$L(D)$$ 曲线，要么每个 $$D$$ 单独跑一次完整调度，要么用常数学习率（或第四篇的 WSD 调度——它的"稳定"段就是常数学习率，随时可以分叉出一段退火拿到一个干净的 $$L(D)$$ 点，这是 WSD 在 2024 年被广泛采用的直接原因之一）。
...
$$\alpha \approx \beta$$ 给出 $$a \approx b \approx 0.5$$：**参数与数据应该同步增长**。直觉是：loss 的两项衰减速度差不多，把算力偏向任何一边都让另一项成为瓶颈。重拟合的常数下 $$G = 0.12$$，$$a = 0.51$$；Kaplan 的 0.73 若硬套进这个加性形式，相当于 $$\beta / \alpha \approx 2.7$$——数据项衰减得比参数项快得多，所以数据可以少给；但 Kaplan 的公式不是这个形式（第二章 §2），这只是一个帮助理解方向的换算。
...
Table: 不同算力下的 Chinchilla 最优 N、D 与 loss
...
```mermaid
%% 图：scaling law 的实验流程：扫一组小模型、逐尺寸调超参、拟合、留出最大点验证、再外推到目标算力
flowchart TB
...
小模型的最优学习率和 batch 大小不是大模型的最优值。Kaplan 的 0.73 里有一部分正是这个原因（第二章第 4 节，Porian 等确认的三因素之一）。两条路：
...
| Kaplan 规则 | $$N \propto C^{0.73}$$ | 差异来自不数输出层算力、固定 warmup、超参不随规模调（Porian 等 2024）；cosine 衰减不是主因 |
...
4. Kaplan（2020）得出 $$N \propto C^{0.73}$$（模型优先），Chinchilla 得出 $$N \propto C^{0.5}$$（数据同步增长）。差异从哪来？
   ...
   Porian 等 2024 复现后归结为三件事：不数输出层（embedding / lm_head）的算力、warmup 步数固定不随训练长度变、学习率与 batch 不随模型尺寸调；逐个修正后与 Chinchilla 一致。常被引用的"固定长度 cosine 调度"被他们专门检验并否定，不是主因。
