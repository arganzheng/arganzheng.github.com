...
flowchart TB
    ...
    subgraph GPU["GPU SM：面积买总吞吐"]
        direction TB
        G1["ALU：64 个 FP32<br/>+ 4 个 Tensor Core"]:::gpu ~~~ G2["寄存器文件 256 KB<br/>整卡 27 MB，与 40 MB 的 L2 同量级"]:::gpu ~~~ G3["小缓存：L1/shared<br/>合计 192 KB"]:::gpu ~~~ G4["4 个顺序发射的 warp 调度器<br/>没有分支预测、没有乱序"]:::gpu
    end
