...
[^q0]: 目标 $$J = \sum_i \|x_i - c_{\text{label}_i}\|^2$$。分配步固定中心，让每个点选最近的中心——对每个 $$i$$ 单独最小化，$$J$$ 不增；更新步固定分配，让每簇中心等于簇内均值——均值是 $$\sum \|x - c\|^2$$ 的唯一最小点，$$J$$ 不增。$$J \ge 0$$ 单调不增、分配方式有限且不会重复，所以有限步收敛——但只到局部最优，初始化决定落在哪个局部最优，故用 k-means++ 与多次重启。详见[第二章第 1 节](#1-k-means)。
