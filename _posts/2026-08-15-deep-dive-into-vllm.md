...
>
> 这就是 TTFT 与 TPOT 之间那笔典型交易——在第 1.3 节我们说过，系统要做的从来不是消灭这种取舍，而是把它调到 SLO 允许的位置上。
...
`KVCacheManager` 还提供了 `watermark` 机制，用于在准入时预留一层显存余量，通过保留一定比例的空闲块作为缓冲，减少"刚放进来就被抢占"的震荡：
