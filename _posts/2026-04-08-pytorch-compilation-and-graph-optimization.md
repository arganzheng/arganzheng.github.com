...
本文用上面这个刻意简单的函数贯穿全文。三个算子、两个中间结果、一个前向一个反向，足以让编译栈的每一站都有事可做，又不会被模型细节淹没。
...
`torch.compile` 的默认路径是一条流水线：

```mermaid
flowchart LR
    PY[Python 函数<br/>字节码] --> DY[① TorchDynamo<br/>捕获]
    DY --> G1[FX Graph<br/>torch 级算子]
    G1 --> AOT[② AOTAutograd<br/>前后向拆分]
    AOT --> G2[FX Graph × 2<br/>ATen 级算子<br/>前向 / 反向]
    G2 --> IND[③ TorchInductor<br/>代码生成]
    IND --> CODE[Triton / C++ 代码<br/>+ 调度代码]
    CODE --> K[④ Triton 编译器 / C++ 编译器<br/>可执行 Kernel]
```

| 站 | 组件 | 输入 | 输出 | 回答的问题 |
|---|---|---|---|---|
| ① 捕获 | TorchDynamo | Python 字节码 | FX Graph（torch 级）+ Guard | 程序里哪部分是可编译的 Tensor 计算？ |
| ② 拆分 | AOTAutograd | torch 级 FX Graph | ATen 级前向图 + 反向图 | 反向怎么提前得到？副作用怎么消除？ |
| ③ 生成 | TorchInductor | ATen 级 FX Graph | Triton / C++ 源码 | 哪些算子合并成一个 Kernel？内存怎么分配？ |
| ④ 编译 | Triton / 系统编译器 | 源码 | 机器码 | 真正的 GPU / CPU 指令 |

第四站不属于 PyTorch，本文只在需要时提及。

### 3. 交汇点：FX Graph

流水线三个 PyTorch 组件之间传递的是**同一种数据结构**：`torch.fx.Graph`。Dynamo 产出它，AOTAutograd 消费并再次产出它，Inductor 消费它。它是三个组件唯一共享的契约——就像第五篇里 Operator Table 是开发态与运行态的交汇点。

理解 FX Graph 是理解整条流水线的前提。所以第二章先讲它，之后再依次进入三站。

### 4. 横切机制：边界与复用

流水线回答“编译一次做什么”。但还有一组问题不属于任何一站：
...
这四个机制决定的是**流水线何时运行、运行多少次、结果能否重用**。它们贯穿整个编译栈，不是某一站的局部细节。第六章统一讨论。
...
混淆它们会导致一个常见误解：“`torch.compile` 就是先 `symbolic_trace` 再优化”。不是。`symbolic_trace` 无法处理数据依赖的 Python 控制流，Dynamo 正是为了解决这个问题才在字节码层重新实现了捕获。第二章讲数据结构时会用 `symbolic_trace` 做演示（因为它最简单），第三章解释为什么编译栈不能用它。

### 6. 本文的章节安排

```text
二        中间表示：FX Graph——所有组件共享的数据结构
三 ~ 五   流水线三站：Dynamo 捕获 → AOTAutograd 拆分 → Inductor 生成
...
流水线每过一站，图的**结构不变，词汇下降**：从用户写的 Python API，降到第五篇讨论的 `aten::` Schema 层，再分解到更小的核心算子集。后端只需支持核心集就能覆盖所有上层 API——这是 Composite 算子思想在编译器里的延续。
...
## 三、第一站：TorchDynamo 捕获

### 1. 为什么 `symbolic_trace` 不够

`Proxy` 只能记录对它自身的操作。遇到依赖 Tensor **值**的 Python 控制流，它就无能为力：

```python
def g(x):
    if x.sum() > 0:       # Proxy 无法转换成 bool → TraceError
        return x * 2
    return x - 1
```

而真实模型里到处都是这类代码：根据输入长度选择分支、根据配置决定是否走某一层、在循环中依赖 Tensor 形状。此外 `symbolic_trace` 是全有或全无的——遇到无法追踪的代码只能报错，不能“把能编译的部分编译，剩下的保持 Eager”。
...
给 `torch.compile` 传一个自定义 backend，可以直接观察 Dynamo 捕获到的图：
...
结构与 `symbolic_trace` 的结果几乎相同——因为对这个没有控制流的函数，两种追踪器看到的是同一件事。差别在 `placeholder` 的命名（`L_x_` 表示“局部变量 x”，来自 Dynamo 对帧的分析），以及这张图现在**附带了 Guard**。

`backend` 参数说明了一件事：Dynamo 只负责捕获，捕获之后交给谁是可配置的。默认是 `"inductor"`；`"eager"`、`"aot_eager"` 用于调试；也可以接入第三方编译器。
...
Dynamo 的图只有前向，并且是 torch 级的。它不知道反向长什么样，也不区分 `torch.relu` 和 `torch.nn.functional.relu`。把它变成后端可用的东西，是下一站的工作。
...
## 五、第三站：TorchInductor 代码生成

### 1. 输入与输出

Inductor 是默认后端。它接收 ATen 级的 FX Graph（前向图和反向图各处理一次），输出**一个 Python 源文件**，内含：
...
| Extern Kernel | mm、conv、attention | 调用 cuBLAS / cuDNN 等库，**不参与融合** |
...
Triton 不是 Inductor 的唯一目标。CPU 路径生成 C++（配合 OpenMP 和向量化 intrinsics）。第六篇讨论的手写 CUDA Kernel 和 Triton Kernel 是两种不同的“自定义算子实现方式”，前者控制力更强，后者开发效率更高。
...
- 默认不把 `add`/`relu` 融合进 cuBLAS 的 `mm`——库调用是黑盒。`max-autotune` 模式下 Inductor 可以用 Triton 模板生成自己的矩阵乘并把后续 pointwise 作为 epilogue 融合进去，但这是可选路径。
- 不改变数值语义（浮点结合顺序的差异除外）。融合后的结果与 Eager 应当在浮点误差范围内一致。
- 不消除 Kernel launch 本身。`mode="reduce-overhead"` 会额外用 CUDA Graphs 把整个 `call()` 的 launch 序列录制下来回放，这是另一层机制，第八篇讨论。

到这里，流水线的三站已经走完：一段 Python 变成了两个 Kernel。接下来的问题是：这份编译结果什么时候能用，什么时候不能用。
...
修复思路是把“Python 侧的动态”改成“Tensor 侧的动态”：`if cond: a else: b` 改为 `torch.where(cond, a, b)` 或 `torch.cond`；`.item()` 尽量后移到编译区域之外；自定义算子按第六篇的方式注册。
...
一次完整的冷编译——捕获、拆分、生成、Triton 编译——对小函数是秒级，对大模型可能是分钟级。缓存分几层：
...
| Triton Kernel 缓存 | Triton 源码到 PTX/cubin 的编译结果 | 磁盘，跨进程 |
...
以上是默认配置下的路径。同一套组件还能组成其他路径：

| 路径 | 组成 | 适用场景 |
|---|---|---|
| `backend="eager"` | 只有 Dynamo 捕获，图原样执行 | 排查 graph break，确认捕获正确 |
| `backend="aot_eager"` | Dynamo + AOTAutograd，不用 Inductor | 排查 AOTAutograd 阶段的问题 |
| `mode="reduce-overhead"` | 默认路径 + CUDA Graphs | 小 batch、launch 开销占主导的推理 |
| `mode="max-autotune"` | 默认路径 + Triton 矩阵乘模板与调优 | 追求峰值性能、可接受更长编译时间 |
| `torch.export` | Dynamo 捕获整图（不允许 graph break）→ `ExportedProgram` | 序列化模型、脱离 Python 部署 |
| AOTInductor | `torch.export` + Inductor 生成 C++ 与 Kernel → 共享库 | C++ 推理服务，无 Python 运行时 |
...
HotSpot 的 C2 直接生成机器码。Inductor 不生成机器码，它生成 Triton 源码，再由 Triton 编译器（内部经 MLIR、LLVM）生成 PTX。这更像一个编译器把另一种高级语言作为目标，再交给第二个编译器——类似早期把 C 作为目标语言的编译器。理解这一点有助于定位问题：生成代码不对是 Inductor 的问题，生成代码对但 Kernel 慢可能是 Triton 编译或调优的问题。
...
```mermaid
flowchart LR
    PY[Python 字节码] --> DY[Dynamo<br/>捕获]
    DY --> FX1[(FX Graph<br/>torch 级)]
    FX1 --> AOT[AOTAutograd<br/>拆分 · 函数化 · 分解]
    AOT --> FX2[(FX Graph × 2<br/>ATen 级)]
    FX2 --> IND[Inductor<br/>融合 · 内存规划 · 生成]
    IND --> CODE[Triton / C++]
    GB[Graph Break] -.-> DY
    GD[Guard] -.-> DY
    DS[Dynamic Shape] -.-> DY
...
| 站 | 输入 | 输出 | 节点词汇 | 观察手段 |
|---|---|---|---|---|
| Dynamo | Python 字节码 | FX Graph + Guard + 改写字节码 | `torch.*`、`operator.*` | `backend=` 自定义、`TORCH_LOGS="graph_code"` |
| AOTAutograd | torch 级图 | 前向图 + 反向图 + `autograd.Function` | `torch.ops.aten.*` | `TORCH_LOGS="aot_graphs"` |
| Inductor | ATen 级图 | Triton / C++ 源码 + `call()` | 循环级 IR → 源码 | `TORCH_LOGS="output_code"` |
...
| `symbolic_trace` vs Dynamo | 前者用 Proxy 在 Python 对象层追踪，遇到数据依赖控制流报错；后者在字节码层符号求值，遇到无法捕获的代码切断 |
...
torch._dynamo.explain               有几张图，为什么断
    ...
    → backend="eager" / "aot_eager"  定位问题在哪一站
