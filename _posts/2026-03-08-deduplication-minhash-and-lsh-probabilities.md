...
Table: MinHash 手算：随机排列下 A 与 B 的最小元素
...
Table: 不同相似度文本在不同 k 下的 Jaccard 估计
...
Table: 四组 (b, r) 的 S 曲线取值

曲线过 50% 的位置 $$s_{50} = (1 - 0.5^{1/b})^{1/r}$$ 就是这套参数的**阈值**。**FineWeb 用 $$b = 14, r = 8$$**（112 个 hash）：$$s = 0.5$$ 时候选概率 5%，0.7 时 56%，0.8 时 92%，0.9 时 99.96%，曲线在 0.685 过 50%——**这就是"Jaccard 大于约 0.7 视为重复"的来源**（$$s_{50}$$ 是候选概率过半的位置，不是硬边界：0.6 仍有 21% 成候选、0.8 仍有 8% 漏掉），它不是拍脑袋定的，是这组 $$(b, r)$$ 的 S 曲线中点。
...
3. **候选里有 10 对是碰巧相似的随机对**（236 − 226）——本文的流水线后面接了精确算 Jaccard 那一步，会过滤掉不够相似的；LSH 只负责"不漏太多"，"不错杀"由精确比较负责。注意**生产管线常常没有这一步**（FineWeb、RefinedWeb 同桶即判重、传递闭包后每簇随机留一篇），那时误报会直接误删，$$b$$ 就不能一味往大调。
