...
实际用 hash 函数代替"随机排列"：把每个 n-gram 映成一个大整数，就等于给全体 n-gram 排了一个随机顺序，集合的 MinHash 就是 $$\min_{a \in A} h(a)$$。
...
每个签名相等与否是一次成功概率为 $$J$$ 的伯努利试验，$$k$$ 次的平均是 $$J$$ 的无偏估计，标准差 $$\sqrt{J(1 - J) / k}$$（L0 第八篇的标准误）。真的量一下——同一对文本（$$J = 0.644$$），换 200 组随机系数各估一次：
