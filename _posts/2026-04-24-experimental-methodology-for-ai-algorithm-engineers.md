...
> **一个"A 比 B 好 1.5 个点"的结论，要经过哪几关才算可信？[^q0] 每一关不过会犯什么错？[^q1]**
...
Table: 按 seed 间标准差与效应量算需要的 seed 数
...
[^q0]: 五关：1.5 是不是事先预期的主指标上的变化，而不是七个指标里挑出来的一个（[第二章](#二提假设把我觉得会好变成一个可以被证伪的陈述)）；这类实验的 seed 方差是多少，1.5 是不是超过了 2σ——多数微调任务上 σ 约 0.5–1.5，1.5 个点在边缘（[第四章](#四控制随机性噪声有多大差异多大才算信号)）；如果 A 是从多个候选里选出来的，有没有用独立的 seed 或数据验证过（[第四章](#四控制随机性噪声有多大差异多大才算信号)）；A 与 B 的评测协议是否完全相同、baseline 是否调好（[第六章](#六读论文与复现从报了-5到实际-05)）；这个结论在什么规模上得出、要用在什么规模上，效应的方向与幅度在几个规模上一致吗（[第三章](#三小规模先行什么能外推什么不能)）。五关都过是可信的结论；过了三关是值得进一步验证的线索；一关都没过是一个数字。
[^q1]: 依次是：多重比较（在七个指标里挑一个好的）、把噪声当信号（单 seed 的 1.5 个点）、选择偏差（在验证集上选模型再报验证集分数）、不公平比较（baseline 没调好或协议不一致）、错误外推（小规模成立的改动在大规模上消失或反转）。[第八章](#八一份实验清单)的实验清单把六步六错逐条列成检查项。
