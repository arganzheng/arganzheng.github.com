...
一个是它管理的`BerkeleyJEKeyValueStore`，还有BerkeleyDB的environment，用于创建BerkeleyDB实例(openDatabase)和处理事务(beginTransaction)。
...
`HBaseKeyColumnValueStore`的实现有点冗长，这里就不贴代码了。感兴趣的读者自己看源码，不难理解。
