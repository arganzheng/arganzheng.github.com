...
所以"MMLU 涨 2 个点"的判断顺序是：**先确认协议相同**（同一 harness、同一配置、同一答案抽取），**再看置信区间**（14K 题的 MMLU 上 2 个点勉强显著，1000 题的子集上不显著），**再查污染**（用一份同分布的新题或改写题复测，涨幅是否保持）——三关都过才是能力。
...
**AIME 上 5 个点的差距完全在噪声里**；GPQA Diamond 上要差 7 个点才勉强；MMLU 上 2 个点刚好在区间边缘。Miller 2024（"Adding Error Bars to Evals"）把这些算法整理成了标准做法，包括题目成簇（同一篇文章的多道题不独立）时的聚类标准误——它让有效 $$n$$ 更小。

### 2. 配对比较

比较两个模型时不要各自算区间再看是否重叠——两个模型在**同一批题**上答，题目的难度是共同的，配对能消掉它。统计量是"A 对 B 错"与"A 错 B 对"的题数（McNemar 检验），或对题目做配对 bootstrap：重采样题目、每次算 A − B，看差值的区间是否含零。配对后同样的 $$n$$ 能检出小得多的差距——两个模型在 80% 的题上同对同错时，有效的样本只是剩下 20% 里的不一致题，但方差也只来自它们。
...
τ-bench 定义 pass^$$k$$：同一任务跑 $$k$$ 次**全部**成功才算过，衡量一致性。pass^1 70% 的模型 pass^8 可能只有 30%——每次 40% 的失败概率累积。产品里用户不会只试一次，pass^k 更接近真实体验；报告时两个都要给。
...
八篇讲完，把所有方法放回策略、奖励、参考三件套：

| 篇 | 方法 | 奖励从哪来 | 参考怎么约束 | 采样 | 模型数 |
|---|---|---|---|---|---|
| 一 | SFT | 标注的目标序列 | 无（小 lr、LoRA 隐式） | 否 | 1 |
| 二 | RM | 人 / AI 的成对偏好 → Bradley-Terry | — | 否 | 1 |
| 三 | PPO / GRPO | 学出来的 RM | KL 惩罚 | 在线 | 4 / 3 |
| 四 | DPO 一族 | 偏好对，折进 $$\beta\log(\pi/\pi_{ref})$$ | 对数比 | 离线 / 半在线 | 2 → 1 |
| 五 | RLVR | **验证器** | 极弱或无 | 在线 | 2–3 |
| 六 | Agent RL | 验证器（环境状态） | 无 | 在线，异步 | 2–3 + 环境 |
| 七 | 蒸馏 | **教师分布** | — | 序列级否 / on-policy 是 | 2 |
| 八 | 评测 | —（度量本身） | — | — | — |
...
[^q0]: 三关：先确认协议完全相同（同一 harness、配置、抽取）；再看它是否超出置信区间（14K 题上 2 个点在边缘，任何子集上都不显著）；再用同分布的新题或改写题复测看涨幅是否保持。三关都过才是能力，否则是协议或污染。详见[第三章](#三协议每一个细节都改变分数)、[第四章](#四统计多少题才能区分两个模型)、[第七章](#七污染)。
