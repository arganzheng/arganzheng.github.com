...
算术强度高于 295 的操作受算力限制，低于的受带宽限制。这就是 **roofline 模型**的全部内容：一条水平线（算力上限）和一条斜线（带宽 × 强度），操作的性能被压在两条线下面。L4《Transformer 与 LLM》第二篇把整个模型逐层放到这张图上；Infra 05 系列第一篇从硬件侧讲同一件事。算法工程师需要的是用它判断：**我改的这个结构把瓶颈往哪边推了**。
...
把 batch 加大：权重读一次被 $$B$$ 个 token 共享，强度变成 $$B$$ FLOP/字节；算的时间 $$2NB / 989\text{T}$$ 随 $$B$$ 线性涨，搬的时间不变。脚本算了几个点：

```text
batch    1: 带宽时间 4.8 ms, 算力时间 0.0 ms → memory-bound,   209 token/s
batch   32: 带宽时间 4.8 ms, 算力时间 0.5 ms → memory-bound,  6675 token/s
batch  128: 带宽时间 4.8 ms, 算力时间 2.1 ms → memory-bound, 26700 token/s
batch  512: 带宽时间 4.8 ms, 算力时间 8.3 ms → compute-bound, 61582 token/s
```
...
prefill 把 prompt 的 4096 个 token 一起过模型：$$2 \times 8.03 \times 10^9 \times 4096 \approx 66$$ TFLOP，权重只读一次，强度是 decode 的 4096 倍，远高于 295。时间由算力决定：$$66 / 989 \approx 67$$ ms。训练同理（每个 batch 几百万 token 一起过），所以训练的效率指标是 **MFU**（Model FLOPs Utilization：实际有效 FLOPs / 峰值 FLOPs），好的训练能到 40–50%——剩下的一半花在通信、数据加载、小算子、等待上。
...
| 梯度与优化器状态 | 可训练参数量 × 14 字节（上一篇） | 全量微调时的大头 | 无 |
