...
"**分布**"是词表上非负、和为 1 的一组数；"**条件**"是给定前文 $$x_{<t}$$ 之后的分布 $$p(x_t \mid x_{<t})$$；"**语言模型**"是对链式法则 $$p(x_{1:T}) = \prod_t p(x_t \mid x_{<t})$$ 里每一项的参数化（第四章）。它决定了：训练目标是让每一项给真实下一个 token 的概率最大（next-token prediction，下一篇的交叉熵）；生成只能逐 token 采样，每步用上一步的结果做条件（所以有 KV cache）；同一个模型换采样设置就是换分布，评测必须固定它们；RL 里的策略 $$\pi(y \mid x)$$ 是同一个条件分布，整条回答的概率是逐 token 概率的乘积。
