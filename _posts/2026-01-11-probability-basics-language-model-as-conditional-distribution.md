...
- **KV cache 能增量追加**：算 $$p(x_{t+1} \mid x_{\le t})$$ 时前文 $$x_{\le t}$$ 与上一步的 $$x_{<t}$$ 只多了一个 token。"条件分布"本身只保证前文不变；要让前面 token 的中间结果（key、value）**不用重算**，还需要架构上的一条性质——因果 attention 里第 $$i$$ 个位置的 key / value 只依赖 $$x_{\le i}$$，后面来了新 token 也不改它。Transformer decoder 满足这一条，所以缓存有效；一个双向编码器就不满足（新 token 会改前面所有位置的表示）。
