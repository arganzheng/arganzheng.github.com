...
> 本文是[《PyTorch 深度实践：从 Tensor 到深度学习运行时》](/deep-dive-into-pytorch.html)系列的第 8 篇（共十篇）。上一篇：[编译执行与图优化](/pytorch-compilation-and-graph-optimization.html)；下一篇：[分布式 PyTorch](/pytorch-distributed-training.html)
...
多 Stream 是第七章 §5 重叠传输与计算的基础。跨 Stream 的依赖需要用 Event 或 `wait_stream` 显式表达，否则会读到未完成的数据——这是多 Stream 最常见的 bug。
...
容差要有依据：fp32 实现之间的差异通常在 `1e-5` 量级；涉及 bf16 时 `1e-2` 是常见起点（第六章 §6 解释为什么）；融合改变了浮点结合顺序，`1e-6` 的容差几乎必然失败。训练场景还应比对若干 step 后的 loss 曲线，而不只是单次前向。
...
### 5. Nsight Compute：单个 Kernel 的内部
...
本章到此结束。把测量结果对应到五类瓶颈，是进入第五至七章的入口：

| 时间线形态 | Profiler 表格特征 | 归类 | 章节 |
|---|---|---|---|
| CPU 泳道密集无空隙，GPU 泳道稀疏、Kernel 短 | `cudaLaunchKernel` 占 CPU 大头；Kernel 数多、平均几微秒 | Launch-bound | 五 |
| CPU 泳道密集，但大部分时间不在算子上 | Python 函数、`DataLoader`、框架逻辑占 CPU 大头 | Python-bound | 五 |
| GPU 泳道密集首尾相接，逐元素算子占 CUDA 时间大头 | `add`、`mul`、`layer_norm`、`softmax` 等累计占比高 | Memory-bound | 六 |
...
对第九章案例里的 Transformer block，`torch.compile` 把每 step 的 Kernel 数从 2100 降到 640。融合同时也是 GPU 侧 memory-bound 的处方（第六章 §4），所以它在两侧都有收益——这是 `torch.compile` 对小模型也常常有效的原因，即使小模型的 GPU 侧根本不是瓶颈。

### 6. 处方三：fused 优化器，合并参数更新
...
限制来自"录制"这个前提：图内的 shape、控制流、内存地址都必须固定。动态 shape 会触发重新录制；图内不能有 CPU 同步（第七章）；每张图占用一份固定的显存池。它最适合**推理**——shape 固定、无反向、launch 占比高——是 LLM 推理引擎的标配。
...
本章会用到 warp、SM、occupancy、shared memory 这几个 CUDA 执行模型的概念，第六篇第七章 §2 有一节最小集介绍；这里默认读者知道它们指什么。
...
| 算子 | 运算量 | 数据量 | AI | 类别 |
|---|---|---|---|---|
| `mm` | 2 × 128 × 32 × 64 = 524k FLOPs | (128×32 + 32×64 + 128×64) × 4 B ≈ 57 KB | ≈ 9 | 接近 ridge point，但规模太小，实际为 launch-bound（第五章） |
...
Occupancy 低的 Kernel 即使 AI 合适也达不到 Roofline，因为访存延迟暴露出来了。这是第四章 §5 Nsight Compute 里 "Achieved Occupancy" 指标的意义。它只在自己写 Kernel 时（第六篇）需要直接关心；用 PyTorch 原生算子和 Inductor 生成的 Kernel 时，这一层通常已经处理好。
...
### 7. 处方五：`set_sync_debug_mode`，找出所有同步点
...
它是否值得取决于：省下的显存能否换来更大的 batch，更大的 batch 是否让 Kernel 从 launch-bound 变成 compute-bound（第五章）——如果能，总吞吐反而上升；如果 GPU 已经饱和，就是纯损失。第九章的案例会给出一个"不值得"的实例。
...
对照第四章 §6 的表：**Launch-bound**（第五章）。batch=8、d=512 下每个 Kernel 太小，GPU 等 CPU。此时优化任何 Kernel 都无效。

### 4. 修改一：增大 batch（第五章处方一）
...
瓶颈从 launch-bound 变成了 GPU-bound，其中矩阵乘占六成——**Compute-bound**（第六章）。这才是可以谈 Kernel 性能的起点。
...
正确性：bf16 的 20 步 loss 与 fp32 参考的相对差异在 1% 以内，曲线趋势一致。通过。显存：激活值减半，静态部分不变（第八章 §2）。

### 6. 修改三：`torch.compile`（第六章处方一，兼第五章处方二）
...
checkpoint 后加大 batch 的吞吐（1882）反而低于不 checkpoint 的 batch=64（2207）——GPU 在 batch=64 时已经饱和（第六章 §9），再加 batch 没有 launch 收益，而重算增加了 30% 计算。**这个案例里不值得**。如果基线是 batch=8 时显存就满了，结论会反过来。
...
时间    两条时间线：CPU 提交 / GPU 执行，异步，由较长者决定 step 时间
        CPU 侧      Python-bound / Launch-bound        ← 第五章
        GPU 侧      Memory-bound / Compute-bound        ← 第六章，T ≥ max(数据量/带宽, 运算量/算力)
        两侧之间    Sync-bound                          ← 第七章
空间    显存 = 参数 + 梯度 + 优化器状态 + 激活值 + 工作区 + 缓存    ← 第八章
...
| 症状 | 归类 | 观察工具 | 处方 | 位置 |
|---|---|---|---|---|
| GPU 利用率低，CPU 100%，Kernel 多而小 | Launch-bound | Profiler 表格（`cudaLaunchKernel` 占比）、时间线形态 | 增大 batch、融合、CUDA Graphs、fused 优化器 | 五 |
| GPU 利用率低，CPU 时间不在算子上 | Python-bound | Profiler `with_stack` | 向量化、`torch.compile`、预处理移出循环 | 五 |
| GPU 忙，逐元素算子占比高 | Memory-bound | Profiler 按 CUDA 时间排序、ncu DRAM Throughput | 融合、算法级融合、bf16、数据布局 | 六 |
| GPU 忙，矩阵乘占比高 | Compute-bound | Profiler、ncu SM Throughput | Tensor Core（bf16 / TF32）、减少计算量 | 六 |
| 时间线两侧交替空洞 | Sync-bound | `set_sync_debug_mode`、时间线 | 批量取回、pinned memory + 异步拷贝、避免数据依赖 shape | 七 |
| GPU 大段空白，CPU 停在 DataLoader | 数据加载 | 单独 Benchmark DataLoader | `num_workers`、离线预处理、prefetch | 七 |
...
```text
第二篇 stride / contiguous     → 第六章 §7：访存模式决定 memory-bound Kernel 的实际带宽
第三篇 grad_fn 持有计算图      → 第八章 §6：泄漏几乎总是持有了带 grad_fn 的 Tensor
第四篇 autocast / GradScaler   → 第六章 §5-6：为什么快、快在哪类瓶颈、代价的边界
第五篇 入口 / 分发 / 执行      → 第五章 §1：每个算子在 CPU 上的固定成本由此而来
第七篇 融合与内存规划          → 第六章 §4：融合是 memory-bound 算子的处方
```
