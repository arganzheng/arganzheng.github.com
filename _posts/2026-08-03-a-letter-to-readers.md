...
- 评论区上方有一栏「<i class="fa fa-caret-up"></i> 赞同 N <i class="fa fa-caret-down"></i> · 阅读数 · 评论数 · 分享」，首页每篇文章下面也有同样一栏——觉得文章有用点「赞同」，觉得不行点旁边的小箭头，不用登录，一个浏览器一票，再点取消。这是我唯一能收到的、比阅读数更有信息量的反馈。
...
- **分享这篇文章**：那一栏最右边的「分享」点开是个小菜单——手机上第一项是系统的分享面板（微信、微博、备忘录都在里面）；电脑上有微博、X、LinkedIn 的直达链接，「微信扫一扫」会出一个二维码；「复制链接」发到知乎、掘金、群里都靠它。首页列表里每篇下面也能直接分享。想分享的是某一句话而不是整篇，用上面说的选中文字 → 链接图标。
