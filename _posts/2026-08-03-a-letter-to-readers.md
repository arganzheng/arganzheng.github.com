...
**用鼠标选中正文里的任意一段文字**，选区上方会浮出一个小工具条：「赞」「存疑」「评论」「复制」「搜一搜」和一个分享图标。「复制」就是复制文字；「搜一搜」把选中的词丢给 Google，遇到不认识的名词顺手就能查。点「评论」，段落下方就地展开一个输入框，顶部引用着你选的原文。写点什么，发表。然后你会看到：那段文字下面多了一条虚线（像微信读书的划线），段尾多了一个带数字的小标记；点标记或点高亮，就能展开这段文字下面的全部讨论。
