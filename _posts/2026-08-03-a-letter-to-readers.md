...
```mermaid
%% 图：划线评论的流程：选中一句话、点评论、登录发表，那句话变黄并出现计数标记
flowchart LR
...
导航栏的 [Moments](/moments/) 是碎碎念：写不成文章的一张图、一句话、一首歌，按月排成一条朋友圈式的时间线。

- **图**：单张大图，两张或四张双列，三张及以上自动排成三列九宫格方块；点开能滚轮缩放、拖拽查看，没有多余的图题打扰。
- **名言与诗词**：引言卡片保留分行韵律，出处自动靠右对齐。
- **音乐**：放上一行网易云、QQ 音乐、Spotify、Apple Music 链接或音频直链，会直接渲染成播放器卡片，边读边听。
- **互动**：每条随笔底部可以随时点亮红心 ♡（匿名，一个浏览器一票），文字同样支持划线评论，评论区汇总在每个月页面的底部。
- **订阅与直达**：每条随笔都有独立链接（如 `.../2026-09.html#20260921-0802`），便于单独分享；随笔还有独立的 RSS 订阅源：`/moments.xml`。
