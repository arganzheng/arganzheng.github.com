...
- **分享这篇文章**：每篇文章正文末尾有一行分享按钮——手机上点第一个会弹出系统的分享面板（微信、微博、备忘录都在里面）；电脑上有微博、X、LinkedIn 的直达链接，点微信图标会出一个二维码，扫一扫就能发给朋友或朋友圈；最后一个是复制链接，发到知乎、掘金、群里都靠它。想分享的是某一句话而不是整篇，用上面说的选中文字 → 链接图标。
