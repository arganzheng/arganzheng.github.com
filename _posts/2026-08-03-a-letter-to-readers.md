...
- **对一张图评论**：图和图表下面有一行小字图题（「图 N」加说明），鼠标放到图上，图的右上角会出现两个按钮：复制和一个评论小方块。点方块就等于选中了图题（图题会滚到眼前、闪一下），工具条照常弹出来——赞、存疑、评论、建议修改都行。有讨论的图会多一圈淡黄边框。
- **对一段代码评论**：代码块右上角的复制按钮旁边也有同样的小方块，点它就选中了整段代码（免得拖 40 行），工具条照常弹出——赞、存疑、评论、建议修改、复制、搜一搜、分享都行。跑不通的话，点「评论」把系统、版本和完整报错贴进去、勾上 Issue；我的代码大多有版本号和配套的 [ai-learning-labs](https://github.com/arganzheng/ai-learning-labs)，有了环境和报错就能复现。
- **给一章或一节打分**：每个标题（章和小节都有）后面有两个很小的按钮——「<i class="fa fa-thumbs-up"></i> 有用」「<i class="fa fa-question-circle"></i> 没看懂」。不用选文字、不用登录，一个浏览器一票，再点取消。它回答的是划线回答不了的问题：不是哪一句有问题，而是**这一章、这一节整体太难、太水，还是刚好**。手机上选文字不方便，这两个按钮就是给你准备的。
...
**发现错别字、错链接、事实错误怎么办？** 选中那句话 → 「建议修改」（知道怎么改）或「评论」+ 勾「同时提交 Issue」（只知道不对）。懒得打字就点「存疑」再选一个原因；连选文字都懒得选，就点章节标题旁的「没看懂」。这几条是对我最有用的路径，谢谢——它们不会石沉大海：每周一会自动把反馈够多的文章汇成一份「待修订」清单，我（多数时候是我和 AI 一起）照单修，修完那句话下面的线会变成绿色。
