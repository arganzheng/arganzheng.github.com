...
先把六种浮点格式的位域画出来。它们之间的"截断"关系一眼可见：BF16 就是 FP32 的高 16 位，TF32 是 FP32 在乘法器入口丢掉低 13 位尾数，E5M2 是 FP16 的高 8 位；只有 E4M3 是独立设计的指数结构：
...
flowchart TB
    ...
    cast["cast 为 BF16/FP16 副本<br/>(FP32 → BF16 只是截低 16 位)"]
...
[^q0]: BF16 用 3 位尾数换来与 FP32 相同的 8 位指数：动态范围一样，前向、反向的激活与梯度不会溢出或下溢，不需要 FP16 那套 loss scaling 与溢出检查；而训练对相对精度 $$2^{-8}$$ 的噪声不敏感——梯度本来就是随机的。深度学习选范围不选精度。详见[第三章](#三指数位与尾数位的取舍为什么深度学习选范围)、[第四章](#四数值在哪些地方丢失)。
