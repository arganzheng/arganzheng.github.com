...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | 浮点数的一般形式 | 三段位与四类编码、八种格式的位布局、一个直观的比较 |
| 三 | 指数位与尾数位的取舍 | 固定 16 位的两种分法、训练中的数跨多少个数量级、为什么深度学习选范围 |
| 四 | 数值在哪些地方丢失 | 大数吃小数、长求和的误差积累、softmax 的指数溢出、方差计算中的相消 |
| 五 | 累加精度 | k = 4096 的点积在 BF16 中累加会怎样、FP8 Tensor Core 为什么用 FP32 累加器 |
| 六 | 混合精度训练 | 三要素、master weights 为什么不能省、loss scaling、BF16 为什么不需要、训练状态的字节数 |
| 七 | FP8 训练 | E4M3 与 E5M2 的分工、per-tensor 与 delayed scaling、DeepSeek-V3 的分块量化 |
...
先把六种浮点格式的位域画出来。它们之间的"位域包含"关系一眼可见：BF16 的位域就是 FP32 的高 16 位，TF32 是 FP32 在乘法器入口只保留高 10 位尾数，E5M2 的位域是 FP16 的高 8 位；（位域包含不等于"转换就是把低位扔掉"——转换按最近舍入，见 §2）只有 E4M3 是独立设计的指数结构：

```text
bit        31 30    23 22                    0
          ┌─┬────────┬───────────────────────┐
FP32      │S│EEEEEEEE│MMMMMMMMMMMMMMMMMMMMMMM│  1/8/23  bias 127
          └─┴────────┴───────────────────────┘
          ┌─┬────────┬───────────────────────┐
TF32      │S│EEEEEEEE│MMMMMMMMMM.............│  1/8/10  . = 入口截掉, 内存仍 4 B
          └─┴────────┴───────────────────────┘
          ┌─┬────────┬───────┐
BF16      │S│EEEEEEEE│MMMMMMM│                  1/8/7   = FP32 的高 16 位
          └─┴────────┴───────┘

bit        15 14 10 9        0
          ┌─┬─────┬──────────┐
FP16      │S│EEEEE│MMMMMMMMMM│  1/5/10  bias 15
          └─┴─────┴──────────┘
          ┌─┬─────┬──┐
E5M2      │S│EEEEE│MM│          1/5/2   = FP16 的高 8 位, 保留 inf/NaN
          └─┴─────┴──┘
          ┌─┬────┬───┐
E4M3      │S│EEEE│MMM│          1/4/3   bias 7, 无 inf, 仅一个 NaN 编码
          └─┴────┴───┘
```
...
- **BF16** 的位域是 FP32 的高 16 位：指数位与 FP32 完全相同，所以范围一样是 $$\sim 3.4 \times 10^{38}$$，最小正规数一样是 $$2^{-126}$$；代价是尾数只剩 7 位，$$\varepsilon = 2^{-7} \approx 0.0078$$，单位舍入 $$u = 2^{-8} \approx 0.0039$$。FP32 → BF16 的转换是**按低 16 位做最近偶数舍入**（`torch` 与硬件都如此），不是直接扔掉：1.005 的低 16 位超过一半，转 BF16 得 1.0078125，截断则得 1.0——两者都在一个 ulp 内，但方向不同，累加起来偏差不一样。硬件上这一步几乎免费。
...
把它们画在同一根对数轴上：FP16 覆盖 $$[6 \times 10^{-8}, 6.5 \times 10^4]$$，BF16 覆盖 $$[9 \times 10^{-41}, 3.4 \times 10^{38}]$$，E4M3 覆盖 $$[2 \times 10^{-3}, 448]$$，E5M2 覆盖 $$[1.5 \times 10^{-5}, 5.7 \times 10^4]$$。

![各浮点格式可表示正数的范围（对数轴），浅色为次正规区，橙色带为训练中激活梯度的典型分布](/img/in-post/floating-point-formats-and-mixed-precision-ranges.svg)

图里的橙色带是第三节要引用的激活梯度典型分布 $$[2^{-40}, 2^0]$$：它整段落在 BF16/FP32 的窗口中央，却有一大截伸出 FP16 的左端（次正规区甚至更低），而 E4M3 的整个窗口比这条带还窄。下面几节要回答的就是：一个 LLM 的权重、激活、梯度、优化器状态分别落在这根轴的哪一段，以及每一段需要多少个点。

## 三、指数位与尾数位的取舍：为什么深度学习选范围
...
所以取舍很清楚：训练需要范围，不太需要精度。3 位尾数换 3 位指数，让梯度不需要任何缩放就能直接落进可表示区间，同时 BF16 与 FP32 的转换只是对低 16 位做一次舍入。这就是核心问题前半部分的答案：**BF16 不是"精度更差却被选中"，而是它把精度花在了训练不需要的地方省下来，换成了训练需要的范围**。
...
对齐到 2^13 后相加 (| 右侧为 10 位尾数窗口之外):

        1.0011100010 | 000    ← 10^4
      + 0.0000000000 | 001    ← 1 右移 13 位, 唯一的有效位落在窗口外
        ─────────────┼─────
        1.0011100010 | 001    → 窗口外 = 1/8 ulp < 1/2 ulp, 舍掉
      = 1.0011100010   × 2^13  = 10^4
```

同一个例子在 BF16 中更糟：间距是 $$2^{13-7} = 64$$，10000 本身就不可表示（会被存成 9984 或 10048），加 1 到加 31 都没有任何效果。
...
## 五、累加精度：为什么 Tensor Core 用 FP32 累加器
...
flowchart TB
    ...
    fwd["前向 GEMM<br/>输入 BF16, Tensor Core 累加 FP32, 输出 BF16"]
    loss["loss (FP32 标量)"]
    scale["× S  仅 FP16 需要 loss scaling"]
    bwd["反向 GEMM<br/>激活梯度、权重梯度均为 BF16"]
    grad["低精度梯度 g·S"]
    unscale["转 FP32, ÷ S<br/>检查 inf/NaN, 有则跳过本步"]
    opt["Adam 更新 (全 FP32)<br/>m, v 更新, w ← w + Δw"]
    master --> cast --> fwd --> loss --> scale --> bwd --> grad --> unscale --> opt --> master
    fwd -.->|"保存的激活 (BF16)"| bwd
    classDef fp32 fill:#d6eaf8,stroke:#2e6da4;
    classDef low fill:#fdebd0,stroke:#b9770e;
    classDef fp16only fill:#f5f5f5,stroke:#999,stroke-dasharray:4 3;
    class master,loss,unscale,opt fp32;
    class cast,fwd,bwd,grad low;
    class scale fp16only;
```

蓝色节点全程 FP32，橙色节点是低精度；虚线框的 loss scaling 只在 FP16 路径上存在，BF16 训练直接把它拆掉（第 4 小节）。
...
flowchart TB
    ...
    chk{"g·S 中有 inf / NaN ?"}
    skip["跳过本步更新<br/>S ← S / 2<br/>连续正常步数 n ← 0"]
    upd["g ← (g·S) / S, 在 FP32 中<br/>正常更新 master weights<br/>n ← n + 1"]
    grow{"n ≥ growth_interval<br/>(默认 2000) ?"}
    dbl["S ← S × 2<br/>n ← 0"]
    keep["S 不变"]
    nxt["下一步迭代"]
    bwd --> chk
    ...
    classDef good fill:#d5f5e3,stroke:#1e8449;
    class skip bad;
    class upd,dbl good;
```

$$S$$ 的轨迹因此是锯齿形：每 2000 步翻一倍地试探上限，撞到 inf 就立刻减半并丢掉这一步。代价是每步一次全局的 inf 检查（一次 all-reduce 级别的同步）和偶尔的跳步。

### 4. BF16 为什么不需要

BF16 的最小正规数与 FP32 相同，都是 $$2^{-126} \approx 1.2 \times 10^{-38}$$；最大值 $$3.39 \times 10^{38}$$，也几乎与 FP32 的 $$3.40 \times 10^{38}$$ 相同。梯度的分布无论怎样漂移，实际训练中都落在这个窗口里，没有需要平移的理由，loss scaling 整套机制（缩放、检查、跳步、同步）全部省掉。严格说两个边界并不完全一样：BF16 尾数短，非正规数只覆盖到约 $$9 \times 10^{-41}$$（FP32 到 $$1.4 \times 10^{-45}$$），所以 `torch.tensor(1e-42).bfloat16()` 是 0；FP32 的最大值 $$3.4028 \times 10^{38}$$ 转 BF16 会舍成 inf。它们只是离训练中会出现的数值远得没有实际意义。这也是 A100 之后 BF16 迅速取代 FP16 的原因：它不是精度更好，而是少了一个需要调参、会引入跳步和同步的子系统。
...
顺带算一个变体：DeepSeek-V3 报告 3.3.3 节把 Adam 的 $$m$$、$$v$$ 改为 BF16（各 2 B），但 master weights **与用于 batch 累积的梯度都保留 FP32**（各 4 B）；GEMM 输入用的 FP8 权重副本按 1 B 记，合计约 13 字节/参数，671B 约 8.7 TB（早期版本这里按 BF16 梯度写成 11 字节，是误读）。这种"优化器状态降精度"之所以可行，是因为 $$m$$、$$v$$ 只是梯度的滑动平均，对它们做 $$2^{-8}$$ 的相对舍入不会像对权重那样累积——它们每一步都被新梯度以 $$1 - \beta$$ 的权重覆盖一部分。
...
flowchart TB
    subgraph pt["per-tensor scaling: 即时 amax"]
        direction TB
        p1["第 1 遍读 x: 求 amax_t"]
        p2["s_t = 448 / amax_t"]
        p3["第 2 遍读 x: 量化为 E4M3"]
        p4["FP8 GEMM"]
        p1 --> p2 --> p3 --> p4
    end
    subgraph ds["delayed scaling: 历史 amax"]
        direction TB
        h["amax 历史窗口<br/>(最近 1024 步)"]
        s2["s_t = 448 / max(历史)<br/>不读 x 即可得到"]
        q2["唯一一遍读 x:<br/>量化 + FP8 GEMM 融合"]
        e2["epilogue 顺便算出 amax_t"]
        h --> s2 --> q2 --> e2
        e2 -->|"写入历史, 供第 t+1 步用"| h
    end
    classDef extra fill:#fadbd8,stroke:#c0392b;
    classDef fused fill:#d5f5e3,stroke:#1e8449;
    class p1 extra;
    class q2 fused;
```

左边多出的那一遍读取（红色）正是 FP8 想省掉的访存；右边用一个环路把 amax 的计算挪到上一步的 epilogue 里（绿色节点只读一遍 $$x$$），换来的是 $$s_t$$ 总是基于 $$t-1$$ 及更早的统计。
...
k 方向 (DeepSeek-V3: k = 7168 = 56 块 × 128)
          块 0       块 1       块 2               块 55
激活 x  ┌──────────┬──────────┬──────────┬─ ... ─┬──────────┐  1×128 分块
(E4M3)  │ s_a[0]   │ s_a[1]   │ s_a[2]   │       │ s_a[55]  │  每块一个 scale
        └──────────┴──────────┴──────────┴─ ... ─┴──────────┘
权重 W  ┌──────────┬──────────┬──────────┬─ ... ─┬──────────┐  128×128 分块
(E4M3)  │ s_w[0]   │ s_w[1]   │ s_w[2]   │       │ s_w[55]  │  每块一个 scale
        └──────────┴──────────┴──────────┴─ ... ─┴──────────┘
             │          │          │                  │
             ▼          ▼          ▼                  ▼
Tensor  ┌──────────┬──────────┬──────────┬─ ... ─┬──────────┐  每块 4 条 WGMMA
Core    │ P_0      │ P_1      │ P_2      │       │ P_55     │  (k = 32×4 = 128)
(14 位) └────┬─────┴────┬─────┴────┬─────┴─ ... ─┴────┬─────┘  然后累加器清零
             │          │          │                  │
             ▼          ▼          ▼                  ▼
CUDA    acc += P_0 × s_a[0]·s_w[0] + P_1 × s_a[1]·s_w[1] + ... + P_55 × ...
Core    (FP32 累加, 深度 56; 每段内 14 位精度的累加深度只有 128)
```
...
V3-fp8-recipe 13 B/param  state=8.72 TB  {'weight_copy': 1, 'grad': 4, 'master': 4, 'moment1': 2, 'moment2': 2}
...
[^q0]: BF16 用 3 位尾数换来与 FP32 相同的 8 位指数：动态范围几乎一样（非正规数区间更窄、最大值略小，但离训练会遇到的数值很远），前向、反向的激活与梯度实际不会溢出或下溢，不需要 FP16 那套 loss scaling 与溢出检查；而训练对相对精度 $$2^{-8}$$ 的噪声不敏感——梯度本来就是随机的。深度学习选范围不选精度。详见[第三章](#三指数位与尾数位的取舍为什么深度学习选范围)、[第四章](#四数值在哪些地方丢失)。
[^q1]: 更新会被舍掉。每步的 $$\Delta w / w$$ 在 $$10^{-4}$$ 到 $$10^{-3}$$ 量级，低于 BF16 的相对精度 $$2^{-8} \approx 0.004$$，$$w + \Delta w$$ 会被舍回 $$w$$（$$1.0 + 0.001 \to 1.0$$），参数根本不动，训练停滞；所以必须保留 FP32 master weights，在 FP32 上做更新再转回 BF16 副本参与计算——这就是显存账里每参数 4 字节主权重的来源。同一个道理管着累加：点积必须在比输入高得多的精度里累加，FP8 Tensor Core 约 14 位的累加精度迫使 DeepSeek-V3 每 128 项提升到 FP32。详见[第五章](#五累加精度为什么-tensor-core-用-fp32-累加器)、[第六章](#六混合精度训练)。
