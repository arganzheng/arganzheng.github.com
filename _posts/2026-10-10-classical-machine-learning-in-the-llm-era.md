...
```text
第二章   数据集与泛化          划分、泄漏与污染、过拟合与 reward hacking、偏差 - 方差与投票
第三章   监督学习              逻辑回归是奖励模型的骨架；分类器是数据管线的主力；树模型；KNN；算过滤器的算力账
第四章   无监督学习            聚类与多样性、去重：MinHash 与 LSH 的概率、PCA 与可视化
第五章   评估                  混淆矩阵到 AUC；judge 的一致性；校准；类别不平衡
第六章   自测清单              四件做出来的事
第七章   怎么学                材料与顺序
第八章   本文小结
```
