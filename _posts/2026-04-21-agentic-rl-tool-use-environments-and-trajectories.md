...
最后一条是 off-policy 的来源，修正靠第三篇的重要性比（按轮算，每轮记录生成它的策略版本的 $$\log \pi_{old}$$）与限制"落后步数"（缓冲区里超过 $$k$$ 步的轨迹丢弃）。$$k$$ = 1–4 在多数报告里几乎无损。**Infra 地图里"RL 训练系统"选修的最难部分——异步 rollout、多版本权重、按轮的 off-policy 修正——是被 Agent 环境逼出来的**，单轮 RLVR 只是让它变得划算，多轮让它变得必需。
