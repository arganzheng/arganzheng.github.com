...
![同一个排好序的分布上，top-k、top-p、min-p 三种截断各砍在哪里](/img/in-post/decoding-strategies-top-k-top-p-min-p.svg)
...
pass@k 是 $$1 - (1 - q)^k$$。当 $$q$$ 小时（难题），$$1 - (1-q)^k \approx kq$$，与 $$q$$ 线性；当 $$q$$ 大时（易题）它已接近 1，对 $$q$$ 不敏感。所以 pass@k 主要由难题决定，而难题上 $$q(T)$$ 可能随温度上升——这就是 pass@k 随温度升高而**上升**的机制。**题目的异质性就足以解释这个现象**，不需要别的假设：两道题，$$T = 0.6$$ 时正确率 $$(0.99, 0.001)$$，同样的 logits 在 $$T = 1$$ 时变成 $$(0.94, 0.016)$$——平均 pass@1 从 0.496 降到 0.478，平均 pass@64 却从 0.53 升到 0.82（CPU 算例）。逐题看两个指标始终与 $$q$$ 同向，只有跨题平均才出现"相反响应"。

有一个流行的解释要澄清："低温下 $$k$$ 次采样高度相关、有效样本数远小于 $$k$$，所以公式失效"——这**不成立**。给定 prompt 与温度，每次采样都是从同一个分布独立抽的，$$1 - (1-q)^k$$ 在任何温度下都精确；低温下 $$k$$ 条输出**相似**，是因为分布本身尖锐、$$q$$ 接近 0 或 1，不是因为它们统计上相关。"相似"是分布的性质，"相关"是采样的性质，两者不能混。

实证上，代码生成的经验（Codex 论文）：pass@1 最优温度约 0.2，pass@100 最优温度约 0.8。数学推理类似。所以：**pass@1 与 pass@k 的最优温度通常不同，各自要单独调**，并且要在验证集上调、在测试集上报；一份评测报告若只给一个温度下的 pass@1 与 pass@100，很可能其中一个不在最优点。但"不能用同一组参数"不是定律——同一温度下报告两条曲线本身是有价值的（它就是本章的实验），只是别把某一个温度下的数当作两个指标各自的上限。
...
- 扫描：temperature ∈ {0（greedy）, 0.2, 0.6, 1.0, 1.4} × 截断 ∈ {无, top-p 0.95, min-p 0.05}；每个配置对每题采 $$n = 16$$ 次（vLLM 的 `SamplingParams(n=16)`，prefix 共享让成本可接受）。temperature = 0 那一格例外：vLLM 对 greedy 要求 `n = 1`（`_verify_greedy_sampling` 直接报 `ValueError`），greedy 本来也只有一条输出，那一格跑一次、pass@16 就等于 pass@1。
...
[^q0]: pass@1 **下降**，pass@64 **上升**。温度升高分布变平，单次采样落到正确序列上的概率下降；但对难题（正确序列在低温下概率极低、$$q \approx 0$$）升温让 $$q$$ 上升，$$1-(1-q)^{64} \approx 64q$$ 跟着涨；易题的 pass@64 早已接近 1 不再变。两个指标对单题始终同向，跨题平均才反向——不是采样"变独立"了，每次采样在任何温度下都是独立的。详见[第三章](#三变形与截断temperaturetop-ktop-pmin-p)、[第六章](#六多步多次采样与选择)。
