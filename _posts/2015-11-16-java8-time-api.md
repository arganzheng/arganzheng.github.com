...
老的Java 时间API的核心类很少，就下面几个：

* java.util.Date: 表示一个特定的时间，精度是毫秒(millisecond)
* java.text.DateFormat: 格式化时间
	* java.text.SimpleDateFormat
* java.util.Calendar: 表示一个日历时间，比如年月日星期。有时区的概念。
...
* java.time Package: 这个是新Javag Date Time API的基础包。所有核心类都放在这个package下，比如LocalDate, LocalTime, LocalDateTime, Instant, Period, Duration 等等。所有这些类都是不可变并且是线程安全的。大部分情况下，使用这些类就足够完成常见的时间日期需求。
* java.time.chrono Package: 这个package定义了非ISO日历体系的通用API。我们可以扩展其中的类来创建我们自己的日历体系。
* java.time.format Package: 这个package包含用于格式化(formatting)和解析(parsing)日期(date time)对象的类。大部分情况下，我们不需要直接使用它们，因为`java.time` package中的核心类已经提供了formatting和parsing的方法了。
* java.time.temporal Package: 正如名字显示的，这个package包含了一些临时对象，可以让我们用来确定特别的日期或者时间。比如，我们可以用它来查找某个月的第一天或者最后一天。所有的方法基本都是以“withXXX”格式命名。
* java.time.zone Package: 这个package包含了一些类用于支持不同的时区。
...
* Instant: 它代表的是时间戳
* Period & Duration: 时间间隔，类似于MySQL中的Interval。
