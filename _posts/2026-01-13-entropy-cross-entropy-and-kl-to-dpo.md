...
```text
熵       H(p)     = −Σ p log p          p 自己的不确定程度          训练 loss 的下限（数据的熵）
交叉熵   H(p, q)  = −Σ p log q          用 q 编码 p 的平均码长      训练 loss（p = one-hot 时 = −log q(x_t)）
KL       D(p‖q)   = Σ p log (p/q)       用 q 代替 p 多付的码长      RLHF / DPO 的约束项、蒸馏的目标

                   H(p, q) = H(p) + D(p‖q)        交叉熵 = 熵 + KL
```
...
```text
分布                                 熵
确定性：p = (1, 0, 0, 0)             −1·log 1 = 0                        完全确定，没有信息
均匀：  p = (1/4, 1/4, 1/4, 1/4)     −4 · (1/4) log(1/4) = log 4         最大——V 个取值的均匀分布熵是 log V
偏斜：  p = (0.7, 0.1, 0.1, 0.1)     −(0.7 log 0.7 + 3 · 0.1 log 0.1) ≈ 0.94 nat   介于两者之间
```
