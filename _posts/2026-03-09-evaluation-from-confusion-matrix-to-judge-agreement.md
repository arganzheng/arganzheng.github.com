...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | 混淆矩阵与四个指标 | 四个格子；精确率 / 召回率 / F1 各回答什么；12 行手写与 scikit-learn 对数 |
...
**ROC 曲线**是把阈值从 1 扫到 0，每个阈值画一个点（横轴假正率 $$FP / (FP + TN)$$，纵轴召回率）连成的线：阈值 1 时什么都不判正，在左下角 (0, 0)；阈值 0 时全判正，在右上角 (1, 1)。上面手写代码的 ④⑤ 就是这个扫描：按分数从高到低排序，每往下放一个样本就是把阈值降到它的分数，累计的正例数 / 负例数就是当前的召回 / 假正率。
...
一个 benchmark 有 $$n$$ 道题、正确率 $$p$$，每道题对错是一次伯努利试验，正确率的**标准误**是 $$\sqrt{p(1-p)/n}$$（L0 第八篇），95% 区间约 $$\pm 1.96$$ 倍：
...
**独立比较**把两个正确率当成独立的估计，差值的标准误是 $$\sqrt{\text{SE}_A^2 + \text{SE}_B^2} = 3.0\%$$，4.2 个点不到 1.96 倍——不显著。
