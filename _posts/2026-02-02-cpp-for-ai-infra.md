...
这是全系列最重要的一篇。Java 里所有对象都在堆上、变量都是引用、生命周期由 GC 决定；C++ 三者都不同，而且是理解一切后续内容的前提。这一篇按 C++ 初学者的节奏写：先给出一份"读本文需要的 C++ 语法最小集"（类的写法、构造与成员初始化列表、析构、`this` 与 `->`、关键字速查、模板怎么读），之后每个机制都先用十几行能跑的玩具代码和真实输出讲清楚，再看 PyTorch 源码里同一个机制长什么样。

这一篇会覆盖：

- 栈帧、堆与地址的内存图像；栈对象与堆对象；值语义；拷贝发生在哪里；
- 引用 `T&`、常量引用 `const T&`、指针 `T*`，各自用在什么场景；
- `const` 的位置和含义；`const` 成员函数；
- 构造、析构、拷贝构造、拷贝赋值、移动构造、移动赋值——"六大特殊成员函数"；一个 double free 的完整案例与 Rule of Zero/Five；
...
1. 从源码到二进制：[上·编译模型——从一个 .cpp 到可加载的 .so](/cpp-compilation-model-and-project-layout.html) · [下·工程布局——命名空间、库的分层与 CMake](/cpp-project-layout-namespaces-libraries-and-cmake.html)
