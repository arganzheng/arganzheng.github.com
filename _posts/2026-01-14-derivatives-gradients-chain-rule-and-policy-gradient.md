...
读懂本章，这些算法之间的差别就只剩这两个问题。但 GRPO 的"减组内均值"**不能**直接套第 3 节那一行证明——组均值里含 $$R_i$$ 自己，不是"与 $$y$$ 无关的 $$b$$"。一个最小算例：策略是 Bernoulli($$p = 0.5$$)、奖励 $$R = y$$、$$G = 2$$，对 logit 的真实梯度是 0.25，用组均值中心化后的期望梯度是 0.125——正好缩了 $$(1 - 1/G)$$ 倍。所以 GRPO 的估计是**有偏的**（方向对、尺度缩 $$1 - 1/G$$，$$G$$ 大时可忽略），再除以组内标准差又引入一个偏差；严格无偏的做法是留一法（RLOO：每条的 baseline 用其余 $$G-1$$ 条的均值）。L5 第三篇的 Dr. GRPO 讨论的就是这些。另外 PPO 的裁剪限制的是**这一项的梯度**在比值越界后归零，不是更新后比值不会越界的硬保证。
