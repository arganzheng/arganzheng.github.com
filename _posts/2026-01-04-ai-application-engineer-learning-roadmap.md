...
这是三张 AI 学习地图中的第三张（三张的总览与分工见[《AI 全栈学习地图》](/ai-fullstack-learning-roadmap.html)）。第一张[《AI 算法工程师学习地图》](/ai-algorithm-engineer-learning-roadmap.html)面向**造模型的人**，第二张[《AI-Infra 工程师学习地图》](/ai-infra-learning-roadmap.html)面向**跑模型的人**，这一张面向**用模型做产品的人**——AI 应用工程师。它假设读者有后端工程的基础（服务、数据库、API、部署），同时关心产品：什么场景值得做、用户怎么用、效果怎么衡量。
...
Table: AI4业务与 AI4IT 在工具集与验证方式上的差别
...
Table: 数字世界与物理世界在模型、输入、动作与验证上的差别

四个格子**共用全部能力层**——模型作为组件、上下文的组装、动作的执行与权限、评测、运营、人机分工。差别主要归结为三个参数：**动作的可逆性**、**时延约束**、**验证方式**（物理世界还多出安全法规、硬件与传感器这些本图不展开的约束，见文末[「不在地图上的内容」](#不在地图上的内容)）。这三个参数很大程度上决定了一个场景的 agent 能走多远：能自动验证且可回滚的场景（代码能跑测试）允许 agent 自主循环几十步再交给人，这是 coding agent 最先成熟的原因；不能自动验证的场景只能把循环收短、把人放在环内；不可逆且验证成本极高的场景（驾驶、电网控制）则要先把验证做成一门独立的工程——仿真——自主级别才能一级一级往上走。理解这三个参数，比记住哪个场景"火"更重要：它告诉你一个新场景的 agent 化上限在哪。
...
后端工程师习惯的组件（数据库、消息队列）有确定的语义；模型没有。这一层的目标是建立一个准确的组件模型——它能做什么、怎么坏、花多少钱、多慢。这一层已写成系列[《模型作为组件：契约、失效模式与选型》](/model-as-a-component.html)（六篇）：失效模式、API 契约（含推理模型的 thinking / effort / 跨轮状态）、成本与延迟的账、选型、客户端工程，按 2026 年 9 月四家（OpenAI、Anthropic、Google、DeepSeek）的 API 与价目写，用 2023–2026 年的公开事件做案例。下表是这一层的概念清单：
...
Table: 传统应用服务、模型 serving 与 agent 运行时的对比
...
Table: 长程任务的演进阶段与人的位置

每上一级，都是把某类验证从人手里交给系统。不能自动验证的动作（给客户发合同）停在 in the loop；能验证但不可逆的动作（删数据库）停在 on the loop 加二次确认；可验证可回滚的（改代码提 PR、跑数据分析）可以走到 out of the loop。自动驾驶的 SAE J3016 分级可以类比到同一条路径上：L2 大致对应人在环内（驾驶员全程监督、随时接管）、L3 对应人在环上（系统请求时接管）、L4 / L5 对应人在环外。这只是类比——SAE 按"谁执行动态驾驶任务、谁负责兜底"划级，不按验证方式划级；但每升一级在工程上都对应验证体系（仿真里程、影子模式、安全冗余）的一次量级跳跃，而不是模型换了一代。
...
Table: coding agent 的 harness 组件与通用意义
...
Table: 本体驱动业务 agent 的 harness 组件与 coding agent 的对应
...
Table: 自动驾驶栈的 harness 组件与数字世界 agent 的差别
...
| 模型网关 | 使用：接入、路由、fallback | — | 建设：多租户、配额、可观测（11） |
...
| Agent | 编排、工具、harness | agent 能力的训练（RL、工具使用数据） | RL 后训练的 rollout 基础设施（09） |
...
地图的七层各对应一个系列，按 L1 → L7 的顺序发布；每个系列不做配套实验，每篇末尾是一节「实践建议」，案例取自业界的公开事件与开源项目，版本与价目以各篇日期前的公开文档为准。
