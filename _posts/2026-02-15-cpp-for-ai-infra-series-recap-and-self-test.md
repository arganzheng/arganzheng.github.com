...
flowchart TB
    ...
    subgraph E["工程实践（后四篇）"]
        direction TB
        C5["05 宏、静态注册与代码生成<br/>TORCH_LIBRARY 怎么把算子登记进去"]
        C6["06 并发、内存模型、TLS<br/>no_grad 的 thread_local 位"]
        C7["07 与 Python 之间<br/>pybind11、refcount、ABI"]
        C8["08 构建、调试与测试<br/>CMake、ASan、gdb"]
    end
    C3 & C4 --> C5
    C2 --> C6
    C2 & C4 --> C7
