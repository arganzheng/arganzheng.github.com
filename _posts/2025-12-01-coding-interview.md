...
《面试手撕代码》是一组共十九篇的系列文章，面向要参加 AI 算法 / AI-Infra / 后端岗位面试、需要在白板或共享编辑器上**当场写出能跑的代码**的求职者。前十三篇覆盖 LeetCode 中等偏上难度的算法题——按解题模式而不是按数据结构的名字组织，每篇讲清一种模式怎么识别、模板长什么样、三到五道最典型的题怎样从题面推到代码、面试官会顺着问什么；后六篇覆盖 AI 岗位特有的"手撕模型组件"——attention、Transformer block、反向传播、tokenizer、采样、损失函数、经典 ML 与评测指标、以及 Infra 岗的并发与系统题。
