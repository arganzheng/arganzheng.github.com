...
> **为什么规则奖励能训出长思维链，而奖励模型训不出来？[^q0] R1 的四个阶段各在修什么？[^q1] 一个推理模型的 RL 训练，rollout 的 token 数比对话模型多多少？[^q2]**

## 一、总览：奖励换成验证器之后

### 1. 先说答案

**为什么规则奖励能训出长思维链。** 三个原因叠加：（一）它**难 hack**——在验证器本身正确的前提下，策略找不到"看起来对但其实错"的捷径，涨分的主要办法是真的答对，所以 KL 约束可以极小甚至为零，策略被允许走得极远。要说清"难"不是"不可"：第二章 §2 列的漏洞（多个 `\boxed{}`、硬编码测试、格式局部最优）都是策略真找到过的；而且答案对不等于过程对、不等于目标完整——策略可以靶向题库的分布、可以牺牲多样性与通用能力（阶段 3 从基座重来就是为此），"确定"消掉的是奖励的噪声，不是 Goodhart；（二）它对长度中立——RM 偏爱长回答是数据里的相关性（第二篇），验证器对长度没有任何倾向，回答变长是因为**更长的推理确实更常答对**，是被选出来的而不是被偏好出来的；（三）基座里已经有这个能力的种子——预训练语料里有大量分步推理的文本，基座在低概率下会生成"等一下，让我重新检查"这类模式，RL 做的是**放大被验证为有效的模式**。学出来的 RM 做不到第一条，也就做不到后两条：策略推到几千 token 之前就先找到了 RM 的漏洞。
...
共同性质：**稀疏**（多数是整条回答一个 0 / 1）、**确定**（没有噪声——同样的回答永远同样的分）、**只在有标准答案的地方存在**。第二篇 RM 那种"学出来的评分器被策略钻空子"的过优化，在这里换了形态而不是消失：奖励确定，所以不会被"看起来好"骗；但验证器只覆盖最终答案，策略推远后仍可能过程胡说、记题库、语言混杂、通用能力退化——第三章的阶段 3/4 和语言一致性奖励就是在补这些。
...
这三条合起来说明：只要奖励可信（验证器正确、难以钻空子）、探索空间足够（长上下文、大 $$G$$、弱 KL）、训练足够久，策略梯度会自己找到长推理这个策略。
...
Table: 对话 RLHF 与推理模型的 rollout 账
...
pass@$$k$$（采 $$k$$ 次任一对）在**题集平均**上随 $$k$$ 近似对数增长——每翻倍 $$k$$ 加固定几个点，然后趋平。单题上它不是对数律：单题成功率 $$q$$ 时 pass@$$k = 1 - (1-q)^k$$，很快饱和到 1；对数增长是不同难度的题（不同 $$q$$）叠加出来的形状，趋平的位置是"$$k$$ 次采样内一次都碰不到解"的题的比例，它随 $$k$$ 还会慢慢降，不是基座的硬天花板。多数投票 cons@$$k$$ 比 pass@$$k$$ 低（要得票最多的答案恰好对，而非任一对），但它是可部署的（不需要知道答案）；R1 的 AIME 71.0（pass@1）→ 86.7（cons@64）是这条曲线上的两个点。成本是线性的：cons@64 在 10K token 的回答上是每题 64 万 token——**64 倍的推理费用换 16 个点**。
...
def extract_answer(text):                        # 第二章第 2 节：只取最后一个 boxed，多个也只取最后
    # 取最后一个 \boxed{...}；用括号计数而不是 [^}]*，否则 \boxed{\frac{1}{2}} 会被截成 \frac{1
    i = text.rfind(r"\boxed{")
    if i < 0: return None
    depth, j = 0, i + len(r"\boxed")
    for k in range(j, len(text)):
        depth += {"{": 1, "}": -1}.get(text[k], 0)
        if depth == 0: return text[j + 1:k].strip()
    return None

def _text(c):                                    # 对话式数据里 completion 是 [{"role": ..., "content": ...}]，先取正文
    return c[0]["content"] if isinstance(c, list) else c

def accuracy_reward(completions, answer, **kw):  # answer 列要先预处理成纯答案（GSM8K 原列含推导与 "####"）
...
def format_reward(completions, **kw):            # 格式分远小于准确分（第二章第 2 节的局部最优）
    ok = lambda c: bool(re.fullmatch(r"<think>.*?</think>\s*<answer>.*?</answer>", _text(c), re.S))
    return [0.1 if ok(c) else 0.0 for c in completions]

cfg = GRPOConfig(num_generations=16, beta=0.0,            # 规则奖励下 KL 可去
                 max_completion_length=4096, loss_type="dr_grpo",   # 第五章第 3 节：只去掉长度归一化偏差；
                 scale_rewards="none",                              # 除以组内 std 是另一个开关，Dr. GRPO 要两个一起关（参数名按 pin 的 trl 版本查）
                 per_device_train_batch_size=16,                    # 生成 batch 必须能被 num_generations 整除，trainer 会显式检查
...
| 项 | 规则 / 事实 | 数字 |
|---|---|---|
| 可验证奖励 | 数学答案等价、代码测试、格式、约束、带参考的判定；确定、稀疏、难 hack（漏洞在验证器与"答案对≠过程对"） | KL 可到 0（R1 的选择，不是必然） |
...
1. 规则奖励下为什么可以把 KL 系数 $$\beta$$ 设为 0？RM 奖励下为什么不行？
   ...
   规则奖励难 hack（答对才有分），策略离参考再远也很难"骗到分"，所以 R1 选择把 KL 撤掉——这是配方取舍，不是逻辑必然：KL 还保护通用能力与多样性，R1 阶段 3 从基座重训、阶段 4 对通用 prompt 恢复 RM + KL 正是在补这个；RM 奖励可 hack，KL 是防止策略走到 RM 失效区域的主要约束。
...
[^q0]: 因为规则奖励难 hack（验证器正确的前提下）——策略涨分的主要办法是真的答对，KL 可以撤掉，探索可以走到几千上万 token；它对长度中立，回答变长是「更长的推理更常答对」被选择出来的结果；而基座里已有推理的种子，RL 放大它。学出来的 RM 在策略走到那么远之前就先被找到漏洞。详见[第二章](#二可验证奖励)、[第三章](#三r1-zero只用规则奖励从基座直接-rl)。
[^q1]: 冷启动修可读性与语言混杂；推理 RL 把能力推到顶并压混语；用 RL 产出的 80 万条重新 SFT 基座，把推理与通用合到一个模型；全场景 RL 补偏好对齐。详见[第四章](#四r1-的四阶段)。
[^q2]: 多 8 到 64 倍：KV cache 从 GiB 到 TiB 只能分波生成，单条几万步的 decode 让一步的墙钟由最长的回答决定——推理模型的 RL 训练在系统上是一个长序列生成问题。详见[第五章](#五长思维链的成本)。
