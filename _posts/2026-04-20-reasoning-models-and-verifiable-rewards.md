...
[^q0]: 因为规则奖励不可 hack——策略涨分的唯一办法是真的答对，KL 可以撤掉，探索可以走到几千上万 token；它对长度中立，回答变长是「更长的推理更常答对」被选择出来的结果；而基座里已有推理的种子，RL 放大它。学出来的 RM 在策略走到那么远之前就先被找到漏洞。详见[第二章](#二可验证奖励)、[第三章](#三r1-zero只用规则奖励从基座直接-rl)。
