...
第一篇全文——它是后面六层的索引。第二篇的一、二、三章（共同骨架、工具调用的往返、结构化输出）。第四篇的一、二章（成本公式与四家价目）。第五篇的一、二章（榜单为什么失真、评测集怎么开始）。第六篇的一、二章（错误分类与重试）。约两小时，建立组件观。

### 完整学习路径

按 1 → 2 → 3 → 4 → 5 → 6 顺序读，做每篇的自测，最后做第七篇的通关自测。

### 已经上线、要解决具体问题

- 账单超预算：第四篇 → 第三篇（effort）→ 第二篇（缓存前缀）；
- 效果不稳定：第一篇（非确定性、上下文腐化）→ 第五篇（评测集）；
- 供应商换模型 / 弃用通知：第五篇第五章 → 第六篇（切换与降级）；
- 偶发失败与超时：第六篇。

### 技术负责人 / 产品经理

第一篇、第四篇的一二章、第五篇。判断一个场景的模型风险、算清成本、做选型决策。
