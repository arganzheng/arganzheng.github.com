...
**多像算重复**：在 n-gram 集合上用 Jaccard 度量，"Jaccard > 0.7 视为重复"不是拍脑袋——它是 LSH 的 S 曲线 $$P = 1 - (1 - s^r)^b$$ 过 50% 的位置：FineWeb 的 $$b = 14, r = 8$$ 给出 $$(1 - 0.5^{1/14})^{1/8} \approx 0.685$$（第五章）。**阈值怎么定**：反过来——先定想要的阈值与陡度，再解出 $$(b, r)$$；$$r$$ 大更严、$$b$$ 大更松，$$k = br$$ 个签名的存储是 512 字节一段。**为什么不两两比较**：$$n$$ 段文本有 $$n^2/2$$ 对，万亿 token 不可能；MinHash 用 $$k$$ 个最小 hash 无偏估计 Jaccard（标准差 $$\sqrt{J(1-J)/k}$$），LSH 分组让相似的对以高概率撞进同一桶——2000 段 200 万对只剩 267 个候选，再精确比较（第三、四章）。LSH 负责不漏，精确比较负责不错杀。
