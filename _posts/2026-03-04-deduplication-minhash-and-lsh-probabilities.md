...
估计的标准差 $$\approx \sqrt{J(1 - J)/k}$$：$$k = 128$$ 时约 ±0.04。
...
配套代码：本文的数字由 [`classical-ml/05_minhash_lsh.py`](https://github.com/arganzheng/ai-learning-labs/blob/main/classical-ml/05_minhash_lsh.py)（纯 NumPy）产生；复现时去拉它。
...
2. 为什么 MinHash 相等的概率恰好是 Jaccard？用一句话说出证明的关键。
   ...
   $$A \cup B$$ 里 hash 最小的元素在 $$A \cap B$$ 里的概率 = 交的大小 / 并的大小，因为随机 hash 下每个元素成为最小的概率相同。
...
[^q0]: 在 n-gram 集合上用 Jaccard 度量。「Jaccard > 0.7 视为重复」不是拍脑袋——它是 LSH 的 S 曲线 $$P = 1 - (1 - s^r)^b$$ 过 50% 的位置：FineWeb 的 $$b = 14, r = 8$$ 给出 $$(1 - 0.5^{1/14})^{1/8} \approx 0.685$$。详见[第二章](#二jaccard)、[第四章](#四lsh)。
[^q1]: 反过来定——先定想要的阈值与陡度，再解出 $$(b, r)$$；$$r$$ 大更严、$$b$$ 大更松；$$k = br$$ 个签名的存储是 512 字节一段。详见[第四章](#四lsh)、[第六章](#六工程上的几件事)。
[^q2]: $$n$$ 段文本有 $$n^2/2$$ 对，万亿 token 不可能。MinHash 用 $$k$$ 个最小 hash 无偏估计 Jaccard（标准差 $$\sqrt{J(1-J)/k}$$），LSH 分组让相似的对以高概率撞进同一桶——2000 段 200 万对只剩 267 个候选，再精确比较。LSH 负责不漏，精确比较负责不错杀。详见[第三](#三minhash)至[五章](#五在-2000-段文本上跑一遍)。
