...
> **面对结构、状态表示和执行方式都在快速变化的模型，推理引擎应该在哪一层吸收变化，才能既跟得上模型世界，又不牺牲性能与可维护性？[^q0]**
...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | 痛点 | 为什么推理引擎必须持续适配新模型；差异如何向下游扩散 |
| 三 | vLLM 的核心抽象 | 从模型接入到运行时执行的每一层抽象及其职责边界 |
| 四 | 适配机制的演进 | 硬编码 → Monkey Patch → Custom Op → IR → 组合计算原语 |
| 五 | 适配的边界 | 抽象机制解决了什么、解决不了什么；通用算子与特化算子的博弈 |
| 六 | 新模型接入的完整路径 | 差异分析、结构实现与权重装配、最小可运行路径、运行时适配、验证与特化 |
