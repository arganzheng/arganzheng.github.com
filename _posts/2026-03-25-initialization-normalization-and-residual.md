...
> **一个 64 层的 MLP 不加任何技巧为什么训不动？[^q0] 初始化、归一化、残差三样东西各修了哪一段，缺了哪一样会怎样？[^q1]**

## 一、总览：一条链与三处修补

### 1. 深网络的一条链

$$L$$ 层网络的前向是 $$L$$ 个函数的复合，反向是 $$L$$ 个 Jacobian 的乘积。两件事都是"连乘"，连乘对每个因子的偏离极端敏感：每层把方差乘 0.5，64 层后是 $$0.5^{64} \approx 5 \times 10^{-20}$$；每层的 Jacobian 谱范数是 0.9，128 层后梯度**至多**是 $$0.9^{128} \approx 1.4 \times 10^{-6}$$；是 1.1 则是 $$2 \times 10^{5}$$。深度学习的稳定性问题几乎全部是这个连乘的问题。三种修法：
...
| 章 | 主题 | 内容 |
|---|---|---|
| 二 | 方差的前向传播 | Var(y) = n·Var(w)·Var(x)；ReLU 砍一半；Xavier 与 Kaiming 的推导；算 64 层的衰减 |
| 三 | 梯度作为 Jacobian 的乘积 | 反向的方差传播；谱范数与 0.9^128；初始化为什么只能修初始时刻；0.02 从哪来 |
| 四 | 归一化 | BatchNorm / LayerNorm / RMSNorm 的公式、参数与计算量；BN 为什么不适合序列；LN 的尺度不变性 |
| 五 | 残差连接 | I + J 的恒等通路；残差流方差每层翻倍还是线性增长；GPT-2 的 1/sqrt(2L) |
...
$$y = Wx$$，$$W \in \mathbb{R}^{n_{out} \times n_{in}}$$，元素独立同分布、均值 0、方差 $$\sigma_w^2$$；$$x$$ 的分量独立、均值 0、方差 $$\sigma_x^2$$。$$y$$ 的每个分量是 $$n_{in}$$ 个独立项之和：$$y_i = \sum_j W_{ij} x_j$$，方差相加（L0 数学系列第四篇的高斯性质）：
...
ReLU 把负半轴置零。对均值 0 对称分布的输入，输出的**二阶矩**是输入的一半：$$\mathbb{E}[\text{ReLU}(y)^2] = \frac{1}{2}\mathbb{E}[y^2]$$（严格说砍半的是二阶矩不是方差——ReLU 输出均值不为零，标准正态过 ReLU 后方差是 $$\tfrac12 - \tfrac{1}{2\pi} \approx 0.34$$；Kaiming 的推导用的正是二阶矩，因为下一层 $$\sum w_i x_i$$ 的方差里出现的是 $$\mathbb{E}[x_i^2]$$）。于是"Linear + ReLU"一层让下一层前激活的方差乘 $$\frac{1}{2} n_{in} \sigma_w^2$$，不变的条件是
...
Table: BatchNorm、LayerNorm 与 RMSNorm 的公式
...
1. 一个 Linear 层 $$y = Wx$$，$$W$$ 的元素独立、方差 $$\sigma_w^2$$，输入 $$n_{in}$$ 维、方差 1。输出的方差是多少？后面接 ReLU 呢？由此 Kaiming 初始化的 $$\sigma_w^2$$ 该取多少？
   ...
   $$\text{Var}(y) = n_{in} \sigma_w^2$$；ReLU 把二阶矩砍掉一半，下一层前激活的方差变成 $$n_{in}\sigma_w^2 / 2$$；要让它等于 1，$$\sigma_w^2 = 2 / n_{in}$$。
