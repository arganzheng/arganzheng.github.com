...
《Transformer 与 LLM：结构、算量与数值》是一组共八篇的系列文章，面向不训练模型、但要为模型搭建训练与推理系统的工程师，以及想知道自己的模型在硬件上"花多少钱"的算法工程师。它讲大语言模型的**成本结构**：每一层做多少次乘加、读多少字节、存多少状态，这些数字由哪些超参数决定，以及各种结构上和数值上的改动如何改变这些数字。同一张成本表的训练侧——tokenizer 与词表、算力怎么分给参数与数据、15T token 从哪来、超参表里的数字从哪来——是紧接着的系列[《预训练：从 tokenizer 到训练配方》](/pretraining-from-tokenizer-to-training-recipe.html)的内容。
...
`llm_cost.py` 的八版（每篇一版，各自独立可运行）与各篇的独立实验（RoPE、最小 MoE 层、浮点格式）保存在 [ai-learning-labs/transformer-and-llm](https://github.com/arganzheng/ai-learning-labs/tree/main/transformer-and-llm)，附每个脚本的完整输出。
...
这是两张地图的交点，全栈读者八篇都要读，但可以分两遍：先读参数量、算量与 Roofline、KV cache、数值格式、量化与投机解码五篇（约 7 小时），它们是后训练、推理系统与 kernel 三个方向共同的前置；位置编码、MoE、多模态三篇在读到长上下文、MoE 模型或 L7 时补。
...
八篇是成本表本身；它的训练侧是紧接着的[《预训练：从 tokenizer 到训练配方》](/pretraining-from-tokenizer-to-training-recipe.html)，只关心推理与部署的读者可以不读。
...
loss 变 NaN、量化后输出异常、两个 kernel 的结果对不上——第六篇给出数值丢失的位置和判断标准，第七篇给出各量化方法各自的误差来源；训练中的 loss spike（三个机制、六个开关、监控曲线）在预训练系列第四篇。前五篇按需回查。
...
先有参数量与 $$6ND$$，再转到[《预训练：从 tokenizer 到训练配方》](/pretraining-from-tokenizer-to-training-recipe.html)：词表、算力分配、数据管线、配方。要为一次预训练做容量与 I/O 规划的 Infra 工程师同样从第二、六篇进入那个系列。
...
紧接着的[《预训练：从 tokenizer 到训练配方》](/pretraining-from-tokenizer-to-training-recipe.html)在这三种之上再加一种：**读报告的能力**——打开一份预训练技术报告，能看出它的 tokenizer、$$D/N$$、数据配比与超参表站在哪个时代、每个决定花了多少。
