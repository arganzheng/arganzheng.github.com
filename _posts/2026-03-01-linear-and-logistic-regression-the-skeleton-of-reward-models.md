...
> **奖励模型和逻辑回归是什么关系？[^q0] 为什么它的准确率到 80% 就上不去了？[^q1]**
...
[^q0]: 奖励模型 = 逻辑回归作用在两个回答的特征差上、无偏置——$$P(A \succ B) = \sigma(w^T(x_A - x_B))$$，loss 是 $$-\log\sigma(r_A - r_B)$$，与 $$\sigma(w^T x + b)$$ + 交叉熵是同一个模型；深度学习只改变了特征 $$x$$ 从哪来。详见[第四章](#四逻辑回归)、[第五章](#五奖励模型--逻辑回归)。
[^q1]: 偏好标签本身有噪声——人与人的一致率 70–80%。合成实验里 3000 对拟出的 $$w$$ 与真实相关 0.999，准确率 91.5% 恰好等于标注一致率 91.3%；模型已经学到了能学的全部，剩下的是标注噪声，再往上就是拟合噪声。所以 RM 的准确率上限是标注协议决定的，不是模型容量。详见[第五章](#五奖励模型--逻辑回归)。
