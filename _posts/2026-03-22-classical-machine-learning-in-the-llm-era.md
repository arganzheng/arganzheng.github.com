...
**Jaccard 相似度**：两段文本各切成 n-gram 集合 $$A, B$$，$$J = \lvert A \cap B \rvert / \lvert A \cup B \rvert$$。**MinHash**：对集合用一个随机 hash 函数取最小值，两个集合的最小值相等的概率恰好等于 $$J$$；用 $$k$$ 个 hash 函数得到 $$k$$ 个签名，相等的比例就是 $$J$$ 的估计。**LSH**：把 $$k$$ 个签名分成 $$b$$ 组、每组 $$r$$ 个（$$k = b \times r$$），任何一组全部相等就把两段文本放进同一个桶当候选。两段相似度为 $$s$$ 的文本被判为候选的概率是
