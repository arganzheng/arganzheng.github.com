...
> **过滤器"准确率 95%"是什么意思？[^q0] judge 与人类"一致率 80%"够不够？[^q1] "20 个 benchmark 领先 12 个"算不算？[^q2]**
...
四个格子里的数随阈值变——第三章的图 1 画了同一个分类器在三个阈值下的三张混淆矩阵。
...
**ROC 曲线**是把阈值从 1 扫到 0，每个阈值画一个点（假正率 $$FP / (FP + TN)$$，召回率）连成的线——图 1 右侧那条：阈值 1 时什么都不判正，在左下角 (0, 0)；阈值 0 时全判正，在右上角 (1, 1)；上一节表里的三个阈值是曲线上的三个点。**AUC** 是它下面的面积，取值 $$[0.5, 1]$$，0.5 是对角线（随机）、1 是完美（曲线贴着左上角）。它有一个直觉的解释：**随机取一个正例一个负例，分类器给正例更高分数的概率**。这里 AUC = 0.973，与阈值无关。
...
配套代码：本文的数字与两张图由 [`classical-ml/06_evaluation.py`](https://github.com/arganzheng/ai-learning-labs/blob/main/classical-ml/06_evaluation.py) 的实验产生；复现时去拉它。
...
[^q0]: 只说对了 95% 的样本，没说错在哪一边；有害内容占 5% 时全判「无害」也是 95%。要看混淆矩阵拆出的精确率（拦下来的里多少真有害）与召回率（有害的漏了多少），以及它们随阈值的权衡；类别不平衡时看 AUC 或 F1。详见[第二](#二混淆矩阵与四个指标)至[四章](#四类别不平衡)。
[^q1]: 先与人和人的一致率比——人之间也只有 70–80%，80% 已到上限；再看是随机误差还是系统偏差——位置、长度、自我偏好三种偏差平均不掉，配对检验能测出来。详见[第六章](#六奖励模型与-judge)。
[^q2]: 不一定算。两个一样好的模型各有 50% 概率领先，20 个里领先 12 个的概率不小；每个 benchmark 各做一次检验还要多重比较校正，而且很多 benchmark 差几个点在置信区间内。详见[第八章](#八两个模型怎么比)。
