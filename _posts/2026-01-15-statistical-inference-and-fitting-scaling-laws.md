...
**HumanEval 上差 3 个点算不算**：164 题、准确率约 50% 时，标准误 $$\sqrt{0.5 \times 0.5 / 164} \approx 3.9$$ 个点，95% 区间约 $$\pm 7.7$$ 个点——3 个点在噪声里，分辨不出（第二、三章）；配对比较（同一批题两个模型各自对错）比独立比较灵敏得多，是正确的做法。**scaling law 怎么拟**：把 $$(N, D, L)$$ 的实验点代入 $$L = E + A/N^\alpha + B/D^\beta$$，在双对数坐标上幂律是直线，最小二乘拟出五个常数（第五、六章）。**$$D/N \approx 20$$ 从哪来**：固定算力 $$C = 6ND$$，用拉格朗日乘子对 $$L$$ 求极值，得到 $$N^*, D^*$$ 都随 $$C$$ 的约 0.5 次幂增长，比值由 $$A, B, \alpha, \beta$$ 决定——Chinchilla 拟出来约 20；拟合常数有标准误，所以 20 是一个区间不是常数。
