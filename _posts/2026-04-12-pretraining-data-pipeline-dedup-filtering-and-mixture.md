...
layout: post
series: pretraining
title: "预训练（03）：预训练数据工程：从 Common Crawl 到 15T token，去重、过滤与配比的账"
...
> **Common Crawl 抽出正文后有 240T token 的量级，为什么开源语料只留下 15T、模型打分后不到 1%？[^q0] 被丢掉的九成多是什么、怎么判定的？[^q1] 15T 里 25% 是"数学与推理"，但公开的数学语料只有几千亿 token——这 3.75T 从哪来？[^q2]**
...
Common Crawl 全部快照的原始网页（WARC，约 100 个快照）              约 30+ PB 压缩
  ...
  ├──────────────────────────────────┬────────────────────────────────────
  ▼ DCLM 分支（resiliparse，2013–2022）  ▼ FineWeb 分支（trafilatura，96 快照至 2023）
DCLM-Pool                     240T  100%   过滤 + 每快照 MinHash 去重后
  │ 启发式 + fastText 取前 10%              FineWeb                      15T
  ▼                                          │ 教育价值分类器
DCLM-baseline                 3.8T   1.6%   ▼
                                           FineWeb-Edu（>= 2）           5.4T   36%
                                           FineWeb-Edu 高分子集（>= 3）  1.3T    9%
```

Llama 3 的 15T 训练集是 Meta 自己的管线，报告没有公布它的漏斗刻度；上面两条公开分支只是量级参照，不能反推 Meta "丢掉了 94%"。

漏斗之后是三个决定：**去重到什么粒度**（URL、文档、行、子串，各自清掉不同的重复）、**过滤到多严**（启发式规则零成本但只清明显垃圾，模型打分留下的少但 loss 更低）、**怎么配**（各来源的比例换算成每个来源要跑多少 epoch）。Llama 3 的 15T 是 50% 通用知识、25% 数学与推理、17% 代码、8% 多语言；其中 3.75T 的数学推理远超公开数学语料的几千亿 token，靠的是从网页里用分类器筛出推理密集的文本、多跑 epoch（第二篇：4 个以内几乎无损）、以及合成与改写。
...
Table: RedPajama 与 Dolma 的语料来源构成
...
Table: 模型打分质量过滤的数据集对比

三种打分器的共同点：**打分的模型不需要大**（fastText 是词袋 + 线性层，第一篇"一页史"里的 TF-IDF 时代的技术），大模型只用来给几十万篇样本标注，标注再蒸馏到小模型上跑 150 亿篇。这是唯一让"用 LLM 判断质量"在 240T token 上可行的算术：用 70B 模型直接给 150 亿篇、15T token 打分，是 $$2 \times 70\text{B} \times 15\text{T} = 2.1 \times 10^{24}$$ FLOPs——比训练 Llama-3 8B 本身还贵 3 倍；标注 46 万篇（约 5 亿 token）只要 $$7 \times 10^{19}$$，是万分之一。

效果：FineWeb-Edu 1.3T 训出的模型在 MMLU、ARC 这类知识与推理 benchmark 上明显超过 15T 的 FineWeb 同算力模型，且用更少的 token 达到同样的分数；DCLM 的消融把 fastText 打分列为所有过滤手段中收益最大的一项，超过任何一种去重或启发式规则的组合，DCLM-baseline 7B 用 2.6T token 在 MMLU 上追平了用了更多数据的同期开源模型。代价是**留下的太少**——1.3T 不够训一个 $$D/N$$ 上千的 8B 模型，所以实际配方是高分子集多跑几个 epoch（第二篇：4 个以内折损 7%）、或作为退火阶段的数据（第五章）。

阈值是一个"质量 vs 数量"的连续旋钮。FineWeb-Edu 给出了三档（≥ 2 留 5.4T，≥ 3 留 1.3T），DCLM 试了前 10% / 20% / 30%：**阈值越严，同 token 数下 benchmark 越高，但总量越少**，最优阈值取决于要训多少 token——小模型、少 token 时用最严的档，大模型、多 token 时放宽以避免多跑 epoch。这是第二篇的数据受限公式在过滤上的直接应用。
...
网页重复得厉害：同一篇新闻被几十个站转载，同一个模板生成几百万个页面，同一个快照里同一 URL 抓了多次。Lee 等 2021 在 C4 上发现约 1% 的 token 属于重复 50 次以上的子串，去掉它们后困惑度更低、且模型逐字背出训练数据的概率降低一个数量级。重复的代价有两面：训练上等价于给这些文本多跑几个 epoch（第二篇：收益递减），行为上让模型学会背诵而不是泛化。
...
近似去重之前先做精确去重：把文档（或段落）做归一化（小写、压缩空白、去标点）后取哈希，相同哈希只留一份。150 亿篇文档的 64 位哈希是 120 GB 的纯载荷（哈希集合还要桶与负载因子，实际两三倍），放得进一台大内存机器；注意 64 位哈希在 150 亿个元素上生日碰撞的期望约 6 对——精确去重要么二次校验原文、要么用更宽的哈希。段落级（每篇几十段）就是几千亿个哈希，Dolma 用**布隆过滤器**：一个位数组加 $$k$$ 个哈希函数，插入时置 $$k$$ 位，查询时 $$k$$ 位全为 1 即"可能见过"。假阳性率 $$\approx (1 - e^{-kn/m})^k$$，$$n$$ 个元素、$$m$$ 位。它省内存的前提是**容忍一定假阳性**：最优参数下每个元素约需 $$1.44 \log_2(1/p)$$ 位，$$p = 10^{-15}$$ 要 72 位/元素——比 64 位精确哈希还多，"内存是精确集合的十分之一"这时不成立；Dolma 实际是在几千亿段落上选一个几十 GB 的位数组，对应的 $$p$$ 在 $$10^{-3}$$ 到 $$10^{-2}$$ 量级、误删少量段落可接受。布隆过滤器不能删除、不能列出内容，只回答"见过没有"，恰好是去重需要的全部。布隆过滤器不能删除、不能列出内容，只回答"见过没有"，恰好是去重需要的全部。
...
这是一条 S 曲线，$$P = 1/2$$ 附近的"半高点"在 $$J^* \approx (1/b)^{1/r} = (1/14)^{1/8} = 0.72$$——它是一个概率过半的位置，不是硬阈值：$$J = 0.6$$ 的对仍有 21% 被选作候选，$$J = 0.8$$ 的对也有 8% 漏掉（配套脚本的理论与实测）：
...
Table: MinHash LSH 候选概率随 Jaccard 的变化

$$J < 0.5$$ 的文档对几乎永不比较，$$J > 0.85$$ 的几乎必然比较。$$b$$ 与 $$r$$ 是两个旋钮：总哈希数 $$k = br$$ 固定时，$$r$$ 越大曲线越陡（更接近理想的阶跃）但拐点越高，$$b$$ 越大拐点越低。给定目标阈值 $$t$$，选 $$r$$ 后 $$b = t^{-r}$$；FineWeb 的 14 × 8 对应"改了 25% 以上就当不同文档"（注意 J 是 5-gram 集合的 Jaccard，改 25% 的词打掉的 n-gram 比例更高，所以"改了 25%"只是量级）。RefinedWeb 用 **450 个 band × 20 行**（9000 个哈希，半高点约 0.74，曲线极陡）——它的论文主文写成 "20 buckets × 450"，附录 G.3.1 给的公式 $$1 - (1 - s^{20})^{450}$$ 才是实际配置；若真是 20 band × 450 行，$$J = 0.8$$ 的对被选中的概率是 $$10^{-42}$$，什么都去不掉。Dolma 与 Llama 3 用类似 FineWeb 的量级。曲线两侧的面积是两种错误：半高点以下被选为候选的是**误报**，半高点以上漏掉的是**漏报**（近重复留在训练集里）。误报是否"不损失数据"取决于后面有没有精确比较那一步：FineWeb 与 RefinedWeb 都**没有**——同桶即判重、传递闭包后每个连通分量随机留一篇（下一节），所以误报会直接删掉不该删的文档，宁可 $$b$$ 大一点的代价是这个。
...
3. **聚类**：把所有候选边（同桶的文档对）做并查集（union-find），得到近重复的连通分量；一个转载了一千次的新闻是一个含一千个节点的分量。连通分量是**传递闭包**：A 与 B 同桶、B 与 C 同桶，A 与 C 就被并到一起，哪怕 A、C 本身相似度很低——链越长误删越多，这是不做精确复核的代价。
4. **选择**：每个分量留一篇（FineWeb 与 RefinedWeb 是随机留一篇；也可以按最长、最早或最可信域名），其余删掉。
...
四个粒度加起来，去重是管线里对最终数据量影响最大的一步：FineWeb 的 MinHash 在每个快照内删掉约一半到三分之二的文档。但第 5 节说明"删得越多越好"不成立，第二篇说明适度重复接近无损——**去重的目标不是"没有重复"，是"没有过度重复"**。实践中的均衡点：URL 与精确去重全做（零争议），文档级 MinHash 按快照或按桶做而不做全局，行级去重按桶做，子串级只对代码与许可证类文本做，语义级只在小规模高质量集上做。
...
Table: Llama 3 配比换算成 epoch 与有效折算
...
这张表还有一个隐含的问题：**权重按什么单位算**。按 token 算，中文的 8% 多语言在 Llama 3 的 tokenizer 下（第一篇：中文 1.46 token/字）对应的字符数比同样 token 的英文少一半；按字符或按信息量算，多语言的实际占比更低。DeepSeek、Qwen 这类中英双语模型报告的配比如果按 token 算，中文的"有效占比"要按 tokenizer 效率折算才能与英文为主的模型比。

### 2. 用小模型定权重

权重怎么定？三种方法，成本递增、自动化程度递增。

**代理模型消融**：训一组小模型（第二篇的方法），每组换一种配比，比 benchmark。Llama 3 用"知识分类"把网页按主题分类后调整比例（压低艺术与娱乐），并用小模型消融验证。它的问题是组合爆炸——$$n$$ 个来源的配比空间是 $$n - 1$$ 维，每个点一次训练。
...
| 长上下文 | 长文档为主，上下文分几步扩到 128K | 800B（Llama 3） | 位置外推（《Transformer 与 LLM》第四篇）；attention 项贵，只能放最后 |

Table: 多阶段预训练的数据与目的

**退火**（annealing）阶段的效果被反复验证：Llama 3 报告（3.1.3 节）对 8B 模型在最后 40B token 上退火，GSM8K 提高 24%、MATH 提高 6.4%——但要看清这个实验的条件：那次退火里加进去的是 **GSM8K 与 MATH 自己的训练集**，报告用它来评估"一份新数据有多大价值"（退火当探针），并明确说正式的退火阶段**不含**这些 benchmark 的训练集。所以这两个数字说明的是退火阶段对数据极其敏感、可以拿来做数据消融，不是"常规高质量数学数据能带来 24 个点"。对 405B 同样的实验几乎没有帮助（大模型已经从主预训练里学到了）。OLMo 2 把中期阶段命名为独立的一步，专门构造了一份混合（Dolmino），并发现**用几个不同随机种子跑退火再把权重平均**（model souping）比单次退火更好。MiniCPM 把这做成了 WSD 调度的标准用法（第四篇）：稳定阶段用常数学习率跑全量数据，衰减阶段换高质量数据——衰减阶段的 loss 骤降既来自学习率也来自换数据，两者各占多少要靶向消融（只降 lr 不换数据 vs 两者都做）才知道，MiniCPM 没有给出这个拆分。
...
合成数据的成本是推理 FLOPs：用 70B 模型生成 100B token 是 $$2 \times 70\text{B} \times 100\text{B} = 1.4 \times 10^{22}$$ FLOPs，约 4000 H100 小时——与训练相比很便宜；生成 Phi-4 量级的 400B 是 1.6 万 GPU 小时，仍只是训练算力的百分之一。**合成数据把"数据"变成了可以用算力换的东西**，这是它对数据受限的 scaling（第二篇第四章）的意义。
...
跑三件事：构造 Jaccard 从 0.3 到 0.95 的集合对，验证 $$P(\text{候选}) = 1 - (1 - J^8)^{14}$$ 的 S 曲线（第四章的表）；五篇人造文档（原文、改三个词、加导航栏、同主题另写、无关）的 Jaccard、签名估计与 LSH 判定；同样的改动放到 866 词的长文上看灵敏度如何随长度变化。把 `b, r` 改成 `(450, 20)`（450 个 band × 20 行，RefinedWeb 的实际配置）可以看它的 S 曲线有多陡。
...
[^q0]: 因为 240T 里绝大多数是重复、模板、垃圾与低价值文本，一条漏斗把它过到约 6%：启发式过滤 → 四个粒度的去重 → 模型打分。每一步留下什么都用消融实验验证过——一次 1.8B × 350B token 的消融 2700 H100 小时，差距常达 3–5 分。详见[第二章](#二原料网页有多少)、[第六章](#六污染消融与管线的账)。
[^q1]: 先过启发式规则（Gopher 的 7 条文档规则与重复度、C4 的行级规则，清掉导航、模板、乱码、色情等明显垃圾），再过 URL / 精确 / MinHash（14 × 8，阈值 0.72）/ 行级去重，到 15T 约 6%；再用模型打分——大模型标几十万篇教育价值分，小分类器跑全量——FineWeb-Edu 取 ≥ 3 分只留 8.7%、DCLM 取前 10%，到 1.3–5.4T。判定标准是规则 + 分类器阈值 + 消融实验。详见[第三章](#三过滤两层各清掉不同的东西)、[第四章](#四去重四个粒度)。
[^q2]: 三处：一是**分类器迭代召回**——DeepSeekMath 用种子数据训分类器、从 Common Crawl 里召回、再训再召回，四轮从 14.7B 到 120B；二是**重复**——25% × 15T ÷ 约 0.5T 的独立数据 ≈ 7.5 个 epoch，配比表里的百分比其实是 epoch 数；三是合成数据与退火阶段的高质量数据。「15T」不是 15T 条不同的文本。详见[第五章](#五配比与课程)。
