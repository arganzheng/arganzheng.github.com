...
所以本篇分三段走。先把从硬件到请求的**四层指标**串成一条线，说清每一层从哪采、叫什么名字、哪些名字会骗人；再用这些指标定义**分配率、使用率、有效利用率**三个数，把它们之间的差距分解到具体原因；最后把指标变成**账单**——GPU 小时怎么分摊到团队、每百万 token 的成本怎么算、成本数据怎么回流到配额、切分与扩缩容的参数。作为末篇，最后一章是全系列的总结。

本篇要回答总纲提出的核心问题：

> **一个 64 卡集群上月账单 X 元，DCGM 显示平均分配率 85%、平均 `SM_ACTIVE` 35%。这 50 个百分点的差距分别来自哪里——排队等 gang、训练的通信等待、推理的低峰空转、开发环境的长期占用？[^q0] 每一项对应本系列哪一篇的机制？[^q1]**
...
|---|---|---|---|
| 硬件 | DCGM Exporter（DaemonSet，每节点一个）；读驱动 / NVML / 硬件性能计数器 | `DCGM_FI_DEV_*` · `DCGM_FI_PROF_*` | `--kubernetes`：查 kubelet pod-resources socket，把 `gpu` / `UUID` 映射到 `pod` / `namespace` / `container` |
...
flowchart TB
    ...
    O["OpenTelemetry Collector<br/>trace：网关 → EPP → 引擎；日志：NCCL / XID 模式匹配"]
    G["Grafana 看板<br/>四层同屏"]
    A["Alertmanager 告警<br/>XID · 队列 · TTFT p95"]
    C["成本引擎（OpenCost / allocate.py）<br/>GPU 小时 × 单价 → 按 label / 队列分摊<br/>→ 每百万 token 成本 → 回流到配额 / 切分 / 扩缩容"]
    L --> P
    L --> O
    P --> G
    P --> A
...
| | `DCGM_FI_PROF_SM_ACTIVE` · `DCGM_FI_PROF_SM_OCCUPANCY` | 默认表中**被注释**，需自行开启 | SM 至少有一个 warp / 驻留 warp 比例；本篇的"使用率" |
...
| 容器 / Pod | `kube_pod_container_resource_requests{resource="nvidia_com_gpu"}` | kube-state-metrics | 请求了几张卡（分配率的分子） |
| | `kube_node_status_allocatable{resource="nvidia_com_gpu"}` | kube-state-metrics | 节点可分配卡数（分配率的分母） |
...
| | `vllm:e2e_request_latency_seconds` · `vllm:request_queue_time_seconds` · `vllm:request_prefill_time_seconds` · `vllm:request_decode_time_seconds` | vLLM | 端到端、排队、prefill、decode 各阶段耗时 |
...
| | `vllm:request_queue_time_seconds` · `request_prefill_time_seconds` · `request_decode_time_seconds` · `request_inference_time_seconds` | Histogram；一个请求的时间分解 |
...
[^q0]: 分两层拆。**分配率 85% → 100% 的 15 个点**（卡没分出去）：排队等 gang——32 卡任务凑不齐时空闲的碎片卡（Kueue / Volcano 的 pending 时间与 `ClusterQueue` 的 usage）；MIG 几何浪费的 slice；节点维护、故障隔离的卡；开发环境「预留但没起 Pod」的配额。**分配率里的 `SM_ACTIVE` 35% → 85% 的 50 个点**（卡分出去了但没干活），按 pod label 把 DCGM 的 `SM_ACTIVE` / `SM_OCCUPANCY` 归到任务类型：训练任务的通信等待与气泡（MFU 40% 意味着 SM 忙 40–50%，其余是 all-reduce 等待、PP 气泡、数据加载——`SM_ACTIVE` 长期 40–50% 是训练的正常值，不是浪费）；推理服务的低峰空转（副本数按高峰配、夜间 KV 占用 10% 但卡一直分着）；decode 本身 memory-bound（`SM_ACTIVE` 高但 `SM_OCCUPANCY` 低，正常）；开发环境 / notebook 的长期占用（分配率 100%、`SM_ACTIVE` 接近 0，最典型的浪费）。账怎么算：dcgm-exporter 开 `enablePodLabels`，OpenCost 按 pod_uid 聚合 GPU 时间乘单价，分到 namespace / team；训练按任务、推理按模型 / 租户（经网关的 token 计量对齐）、开发按人。详见[第三章](#三三个数字分配率使用率有效利用率)、[第七章](#七成本模型)、[第十章](#十核心问题50-个百分点去了哪里)。
[^q1]: 排队等 gang → 第三篇（调度：借用、抢占、拓扑）；MIG 几何浪费与开发环境长期占用 → 第四篇（GPU 共享：HAMi / 时间片池 + 空闲回收）；训练的通信等待与气泡 → 07 系列（MFU 拆解），平台侧只负责把 `SM_ACTIVE` 归到任务；推理低峰空转 → 第六篇（缩容阈值与 minReplicas、PD 分离）；成本归因与激励 → 本篇（[第七](#七成本模型)至[九章](#九finops-回路与容量规划)）。分配率与 `SM_ACTIVE` 两条线各画一张：前者是调度问题、后者是负载问题；每类浪费对应一个机制与一个 owner。FinOps 三步：可见（每张卡每小时归到谁）→ 归因（浪费属于哪类）→ 激励（按分配计费让人释放、按 `SM_ACTIVE` 报告让人优化）。
