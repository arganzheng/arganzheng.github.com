...
```text
第一篇  负载画像          一步 = 三个作业 + 两次同步；FLOP 12N（训练占半）· 字节（KV 放不下）· 秒（decode 带宽受限，生成占 3/4）；全步 MFU 上限 13%
第二篇  系统形态          共置 / 分离 / 异步是同一张账上的三种交换；同步分离更差；异步 ≈ 共置 − 长尾；配比按时间不按 FLOP，且会漂
第三篇  共置              显存换手：搬 / 丢 / 常驻三种让渡；CuMemAllocator 保虚拟地址；切换 < 2%，真实代价是 KV 池只有半张卡
第四篇  权重同步          布局（HF 张量流、Megatron-Bridge）+ 传输（IPC / NCCL / RDMA / 中转）；"谁持有完整模型"比链路快慢重要；增量同步 32B–235B 持平
第五篇  异步与 off-policy  三个来源：staleness（与长度相关 → drop 偏短）· 训推不一致（同步下也有）· 缓冲淘汰；三份 logprob；事前记信号
第六篇  Agentic rollout    rollout 变成分布式系统；KV 驻留决定 prefill 二次还是线性；沙箱是第三个池；环境方差让异步成必需
第七篇  verl 源码          一个参数的十二步；@register · 资源池 · hybrid worker · rollout server · checkpoint engine · TransferQueue；slime 薄、AReaL 异步优先
第八篇  配置与排障          六步推导 · 全步 MFU 瀑布 · RL 状态的 checkpoint · 确定性 · 必采指标 · 故障表 · 十分钟决策树
```

### 2. 读完应该能做的三件事

- **算账**：给定模型、任务形态、卡数，算出三段时间与显存、长尾占比、推荐形态与配比、全步 MFU 的预期值，并解释每一项损失——不需要 GPU，需要第一、二篇的公式与第八篇的顺序。
- **实现**：读懂并修改 verl 的显存让渡、权重同步、异步控制、agent loop 接入；或者不依赖框架，把 sleep / wake、bucket + IPC、NCCL 广播、replay buffer 这几段自己写出来——第三到七篇。
- **运维**：为一个持续数天到数周的 RL 任务设计 checkpoint、确定性、指标与告警，在推理引擎、训练器、沙箱集群三方都可能出问题的前提下，十分钟内把一次 reward 异常归到系统或算法——第五、八篇。

### 3. 这个方向在往哪走

写作时（2026 年 9 月）能看到的趋势，每一条都对应本系列的一篇：

- **推理引擎的 RL 接口在标准化**：sleep / wake、按 tag 的显存分区、`update_weights_from_*`、版本号、abort / resume——vLLM 与 SGLang 都在把它们做成一等 API，训练框架的适配层会变薄（第三篇）。
- **权重同步向"没有完整模型"收敛**：增量、多源、点对点；Megatron 侧的增量同步与量化字节的 diff 在路线图上（第四篇）。
- **异步成为默认**：verl v1 的三种模式统一后，`separate_async` 会像 AReaL 那样成为大规模任务的默认；staleness 的准入控制与 decoupled loss 会内建（第五篇）。
- **环境成为平台**：沙箱、工具网关、reward 服务从任务的一部分变成公司级基础设施；"接现成 harness 进训练"（uni-agent 一类）让训练与部署用同一个 agent（第六篇）。
- **可观测的对象从训练器扩到整个循环**：RL-Insight 一类把 rollout、同步、缓冲、环境接进同一面板（第八篇）。

机制层已经稳定到可以写成系列；实现层还在每个版本变。读者拿到的应该是前者——后者查 release note 就好。

### 4. 边界与相邻

本系列没有讨论：RL 算法本身（算法地图的[《后训练》](/post-training-from-sft-to-verifiable-rewards.html)）、训练引擎内部（[《大规模训练工程》](/large-scale-training-from-parallelism-to-fault-tolerance.html)）、推理引擎内部（[《大模型推理系统揭秘》](/deep-dive-into-vllm.html)）、集合通信实现、集群资源层（[《AI 平台工程》](/ai-platform-engineering.html)）。它坐在这四者中间，只讲它们之间的那几条线。
