...
flowchart TB
    ...
    S1 --> S2["训推 logprob 差：涨了？→ 引擎与训练器的数值不一致（第五篇）"]
    S2 --> S3["reward 分布：全对或全错的 prompt 比例？→ 组内没有梯度（GRPO 的方差为零）"]
    S3 --> S4["长尾：最长回答 / 平均回答？→ 一步的墙钟被几条轨迹拖着（第一、六篇）"]
    S4 --> D["这些指标要<b>提前</b>记：出事时才想加就晚了——第六章列全步 MFU 之外该看的每个数"]
