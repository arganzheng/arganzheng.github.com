...
**为什么这个比较是合法的全序**：需要传递性——若 `a+b ≥ b+a` 且 `b+c ≥ c+b`，则 `a+c ≥ c+a`。把 `a+b` 看成数值 $$a \cdot 10^{\lvert b \rvert} + b$$，条件等价于 $$\frac{a}{10^{\lvert a \rvert} - 1} \ge \frac{b}{10^{\lvert b \rvert} - 1}$$——每个数映射到一个实数，比较的是这个实数，所以传递性成立。
...
[^q2]: 定义 `a ≻ b` ⟺ `a + b > b + a`（字符串拼接后比较）。把 `a + b` 看成数值 $$a \cdot 10^{\lvert b \rvert} + b$$，不等式 $$a \cdot 10^{\lvert b \rvert} + b > b \cdot 10^{\lvert a \rvert} + a$$ 等价于 $$\frac{a}{10^{\lvert a \rvert} - 1} > \frac{b}{10^{\lvert b \rvert} - 1}$$，即每个数映射到实数 $$f(x) = x / (10^{\lvert x \rvert} - 1)$$ 后按 $$f$$ 降序——这是一个真正的全序，传递性自动成立，排序结果就是最优拼接。别忘了全零时返回 `"0"`。详见[第三章第 5 题](#5-lc-179-最大数)。
