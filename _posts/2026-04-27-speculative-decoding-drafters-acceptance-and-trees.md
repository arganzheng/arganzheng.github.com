...
> **为什么 EAGLE 的接受率高于 Medusa 而草稿成本差不多？[^q0] 一个 70B 模型该用什么草稿？[^q1]**
...
%% 图：投机解码的一轮：草稿连猜 γ 个，目标模型一次前向验证，全部接受多产一个，第 k 个被拒则重采
flowchart TB
...
采样温度改变 $$p$$ 与 $$q$$ 的形状，也改变它们的重叠。$$T \to 0$$（greedy）时两者都退化为 one-hot，接受当且仅当 argmax 一致——接受率等于草稿的 top-1 准确率。$$T = 1$$ 时是两个完整分布的重叠。中间温度下，两个分布都变尖，如果它们的 argmax 一致，重叠增加；如果不一致，重叠减少。实证上（Leviathan 等 2023 与后续工作的观察）接受率**通常**在 greedy 下最高、随温度上升而下降——因为 argmax 一致的位置占多数，变尖让这些位置的重叠趋近 1。但这不是定理：argmax 不一致的位置在 greedy 下接受率是 0，在 $$T = 1$$ 下却可以接近 1（$$p = (0.51, 0.49)$$、$$q = (0.49, 0.51)$$ 时重叠 0.98）；整体趋势取决于两种位置的比例。这意味着**投机解码在低温下通常收益更大**，而推理模型推荐的 $$T = 0.6$$ 是一个中间位置。

一个常被误解的实现细节：**草稿的采样规则不必与目标一致**。分布等式的证明对任意提议分布 $$q$$ 都成立——只要草稿 token 确实从 $$q$$ 采出、验证时用的 $$q(x)$$ 就是那个分布、拒绝后从 $$\text{norm}(\max(0, p - q))$$ 重采，输出就精确等于 $$p$$（CPU 验证：目标 $$T = 0.6$$、草稿 $$T = 1.4$$，还原 $$p$$ 到 $$10^{-15}$$）。必须固定的是**目标侧**的规则：$$p$$ 是目标在自己的温度 / top-p 之下的分布，它定义了"正确的输出分布"是什么。草稿用不同温度只影响接受率，不影响正确性。vLLM 让草稿与目标共享 `SamplingParams` 是为了接受率与实现简单，不是正确性要求；因此看到草稿与目标温度不同也不能据此判它"改变了分布"。
...
- **on-policy 很重要**。草稿在推理时看到的前缀是目标模型生成的文本（被接受的 token 来自 $$p$$）。所以训练时的前缀不应只来自数据集——DistillSpec 的 on-policy 指的是**草稿模型自己生成**的序列（学生采样，教师在其上打分），与 L5 第七篇 GKD 的定义一致，修的是草稿自己的暴露偏差；被接受的前缀来自目标这一点是另一层考虑。DistillSpec 报告 on-policy 数据优于固定数据集。
...
树上的验证从根开始，按拒绝采样逐层进行：在第 1 层，草稿给了 $$s_1$$ 个候选 $$x^{(1)}, \ldots, x^{(s_1)}$$，它们来自同一个草稿分布 $$q_1$$。**多个候选的拒绝采样**（Miao 等 2024 的 SpecInfer；Sun 等 2024 的 SpecTr 给了更优的形式）：依次对每个候选做接受判定，第一个候选的接受概率是 $$\min(1, p(x)/q(x))$$；一旦拒绝，**剩余分布要整体更新为残差** $$p' = \text{norm}(\max(0, p - q))$$——与链式拒绝后的重采分布相同——再拿下一个候选对 $$p'$$ 与它的提议分布做判定，全部拒绝则从最后的残差采样。这里容易写错的一步是"只从 $$p$$ 里减掉被拒候选那一格的 $$q$$ 质量"：那样得到的不是 $$p$$。一个 3 值例子：$$p = (0.1, 0.4, 0.5)$$、$$q = (0.5, 0.3, 0.2)$$，正确的残差是 $$(0, 0.25, 0.75)$$；只扣被拒的第一格再归一化得 $$(0, 0.44, 0.56)$$，最终输出分布是 $$(0.1, 0.478, 0.422) \ne p$$（CPU 验证）。若候选是无放回抽取或确定性 top-$$k$$，提议分布也要相应改写，SpecTr 处理的正是这一点。这套规则做对了才保证输出严格等于 $$p$$。接受了第 1 层的某个节点后，进入它的子树继续。

期望接受长度在树上没有简单闭式。直觉上多候选提高的是"第 1 层至少接受一个"的概率；但要注意它**不会超过** $$\sum_x \min(p(x), q(x))$$ 对全部 token 的和——那就是链式的 $$\alpha$$（第二章 §2），子集之和只能小于等于它。树的收益不在单层，而在多层同时铺开、每一层都有多个机会，使**整条路径**的期望接受长度增加。EAGLE 报告的链式接受长度约 3.0、树式约 3.8–4.5，是这个效应的量级。
...
1. 接受率 $$\alpha = 1 - \text{TV}(p, q)$$。草稿的 argmax 与目标一致率 90%，接受率一定是 0.9 吗？

   <details markdown="1"><summary>答案</summary>

   不一定。接受率看的是两个分布的重叠（总变差的补），不是 argmax 是否一致：草稿 argmax 对但概率只有 0.4、目标是 0.9，接受率就低于 0.9；反之两者都很平时 argmax 不同也可能接受率高。greedy 解码下接受率最高。

   </details>

2. Medusa 与 EAGLE 的草稿成本都很低（$$c \approx 0.02$$），接受长度却是 2.5–3 对 3.8–4.5。差在哪？

   <details markdown="1"><summary>答案</summary>

   Medusa 的 $$K$$ 个头各自独立预测第 $$k$$ 个未来 token 的边缘分布，头之间没有条件依赖；EAGLE 在特征层自回归——第二个草稿以第一个草稿为条件，且输入了目标模型的隐状态特征而不只是 token。条件依赖 + 特征信息就是差距。

   </details>

3. 树状草稿 64 个节点、batch 8：一次验证要算多少 token？为什么树在大 batch 下更早失效？

   <details markdown="1"><summary>答案</summary>

   $$8 \times 64 = 512$$ 个 token 一次前向；等效 $$m = 512$$ 已过 ridge（295），验证不再“免费”而是 compute-bound；树越大、batch 越大越早撞上。规则：$$B \cdot N_{tree} \lesssim \text{ridge}$$。

   </details>

4. 训练草稿模型的正确目标是什么？为什么“在数据上做 SFT”不是？
...
5. 一个 70B 模型该用什么草稿？给出选择顺序。

   <details markdown="1"><summary>答案</summary>

   先试免费的 n-gram（复制类任务 2–3×，自由生成约 0）；有 MTP 头（如 DeepSeek-V3）直接用，接受率 85–90%、约 1.8×；否则 EAGLE-2/3 一层 decoder 草稿（3.5–6.5×，需要几小时到一天训练）；独立小模型草稿只在没有隐状态可用时考虑。
...
[^q0]: 两者的草稿都是目标模型上的一个极小模块，成本相近；接受率差在两处——EAGLE 的草稿是自回归的，每个草稿 token 以前面的草稿为条件，预测的是条件分布，而 Medusa 的各头相互独立，预测的是边缘分布，边缘分布天然更平、与目标的重叠更小；EAGLE 的输入是目标模型的特征（连续、信息丰富）加上被采样的 token（补上采样的随机性），Medusa 只有最后层的隐状态。EAGLE-2 用草稿置信度动态分配树的分支，EAGLE-3 去掉特征回归的约束并用训练时测试修暴露偏差，把接受长度推到 5–6。详见[第二章](#二接受率的本质)、[第四章](#四medusa-与-eagle草稿看到了什么)、[第五章](#五树状草稿的验证)。
[^q1]: 如果它自带 MTP 头就直接用（约 1.8 倍，零成本）；否则训一个 EAGLE-3 头（1–2 天 8 卡，3 倍以上）；负载有大量复制就先试 n-gram；大 batch 的吞吐服务上投机的收益有限，考虑关掉。详见[第六](#六mtp-作为草稿)至[八章](#八何时投机反而变慢)。
