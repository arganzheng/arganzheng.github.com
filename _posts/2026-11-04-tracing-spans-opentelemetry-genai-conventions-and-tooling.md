...
```text
会话 span（session）
 ├─ 属性：用户 / 租户 · 模型版本 · prompt 版本 · 工具集版本 · 检索配置 · 权限档 · 开始 / 结束 · 总成本
 ├─ 任务 / 轮次 span（turn）
 │   ├─ 属性：用户输入 · 最终输出 · 状态（完成 / 部分 / 失败 / 拒答）· 成本 · 时长 · 反馈
 │   ├─ 模型调用 span（gen_ai chat）
 │   │    完整输入（或引用）· 输出 · usage（输入 / 输出 / 缓存读写 / 推理）· effort · stop_reason · TTFT · 总时长 · 错误
 │   ├─ 工具调用 span（execute_tool）
 │   │    名称 · 参数 · 结果（或卸载引用）· 耗时 · 沙箱 · 审批 · 错误 · 幂等 id
 │   ├─ 检索 span（retrieval）
 │   │    查询 · 过滤 · 候选与分数 · rerank 前后 · 命中的块 id
 │   ├─ 事件：压缩 · 卸载 · 卫士触发 · 审批请求 / 决定 · 决策点（第六篇）
 │   └─ 链接：子会话 id（子 agent）
 └─ …
```
