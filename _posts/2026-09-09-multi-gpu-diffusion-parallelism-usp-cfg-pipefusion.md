...
flowchart TB
    subgraph BEFORE["all-to-all 前：按 token 切（线性层在这个布局上算，无通信）"]
        ...
        B0["卡 0：token 0–N/4
head 0–23"]
        B1["卡 1：token N/4–N/2
head 0–23"]
        B2["卡 2：token N/2–3N/4
head 0–23"]
        B3["卡 3：token 3N/4–N
head 0–23"]
    end
    BEFORE -- "all-to-all（Q、K、V 各一次）：每卡收发 (p−1)/p · N·d/p" --> AFTER
    subgraph AFTER["attention 时：按 head 切（每卡对自己的 head 做完整 FlashAttention，无通信）"]
        direction LR
        A0["卡 0：token 0–N
head 0–5"]
        A1["卡 1：token 0–N
...
head 18–23"]
    ...
    AFTER -- "all-to-all（O 一次）" --> BACK["回到按 token 切：输出投影 · MLP · 下一层"]

    classDef tok fill:#eef6ff,stroke:#5b8fd6,color:#222
    classDef head fill:#fff7e0,stroke:#c98a00,color:#222
    class B0,B1,B2,B3,BACK tok
    class A0,A1,A2,A3 head
```
