...
> 本文是[《大模型推理系统揭秘：从 vLLM 看 LLM Serving Infra 核心技术》](/deep-dive-into-vllm.html)系列的第 9 篇（共十四篇）。上一篇：[Multi-GPU：一张卡不够时如何扩展？](/multi-gpu-scaling-strategies.html)；下一篇：[请求形态的扩展：multi-LoRA 与多模态](/request-shapes-multi-lora-and-multimodal.html)

> **NOTE** 本文基于 vLLM v0.27.1（tag `6e448d0`, 2026-08-11）源码剖析。文中文件路径、类名和函数名均以该版本为准；vLLM 迭代很快，阅读时请以你手上的版本对照。


前面几篇解决的是一个固定模型如何被高效地调度、缓存、执行和扩展。但模型世界变化极快：MLA、MoE、MTP、Mamba、各种量化格式和权重布局层出不穷，而每一种差异都会往下捅穿好几层——Attention Kernel、KV Cache 布局、调度预算、并行与通信都可能被牵动。

本篇的核心问题是：

> **面对结构、状态表示和执行方式都在快速变化的模型，推理引擎应该在哪一层吸收变化，才能既跟得上模型世界，又不牺牲性能与可维护性？**

## 一、总览：抽象吸收变化，特化守住性能

### 1. 本文的思路

本篇先说明为什么推理引擎必须持续适配新模型，以及新模型差异如何向下游扩散；然后系统梳理 vLLM 的核心抽象——从 `ModelConfig`、`ModelRegistry`、能力契约、`ModelLoader` 到 `Worker + ModelRunner`、`SchedulerOutput`、`Attention Metadata` 和 Attention Backend——看它们各自把哪一类变化隔离在哪一层；接着回顾适配机制从硬编码、Monkey Patch 到 Custom Op 和 IR 的演进，并讨论抽象机制的边界：为什么仍然需要特化 Kernel。后半部分转向实践：一个新模型接入 vLLM 的完整路径，以及 DeepSeek 架构（MLA、MoE、MTP）的工程适配案例；最后归纳为模型层、运行时层与算子层的三层适配模型。

### 2. 本文的章节安排

```text
第二章  痛点                      为什么推理引擎必须持续适配新模型；差异如何向下游扩散
第三章  vLLM 的核心抽象           从模型接入到运行时执行的每一层抽象及其职责边界
第四章  适配机制的演进            硬编码 → Monkey Patch → Custom Op → IR → 组合计算原语
第五章  适配的边界                抽象机制解决了什么、解决不了什么；通用算子与特化算子的博弈
第六章  新模型接入的完整路径      差异分析、结构实现与权重装配、最小可运行路径、运行时适配、验证与特化
第七章  DeepSeek 架构的工程适配   MLA、MoE、MTP 各自触及的层次；通用层、特化层与验证层
第八章  本文小结                  推理引擎适配能力的本质
```
...
- 加特殊头 → 影响采样、调度预算 **和** KV 回滚逻辑（见第七章第 3 节 MTP 部分）
...
## 三、vLLM 的核心抽象：从模型接入到运行时执行

在前一章中我们讨论过，模型变化往往会跨层扩散：模型结构会影响状态表示，状态表示会影响执行方式，执行方式又会进一步影响缓存、调度和性能优化。
...
### 1. 配置语义：`ModelConfig` 将外部参数收敛为执行意图
...
### 3. 能力契约：`nn.Module` 加 Protocol，而不是统一继承树
...
### 4. 模型落地：`ModelLoader` 与 `WeightsMapper`
...
### 7. 从执行计划到算子输入：`Attention Metadata`
...
### 8. 模型计算与 Attention Backend：从语义到 Kernel
...
## 六、一个新模型接入 vLLM 的完整路径

前文介绍了 vLLM 的核心抽象及其职责边界。这一章我们从工程实施角度说明**当一个新模型需要接入 vLLM 时，应该按照什么顺序分析、实现、验证和优化。**
