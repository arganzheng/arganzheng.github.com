...
f 和 g 互为**共轭**：一个前向通信、反向不通信，另一个反过来。所以一个 MLP 在训练中通信两次：前向 g 处一次，反向 f 处一次。Attention 同理（下一节）。一个 Transformer 层 = Attention + MLP，**训练中每层每 step 共 4 次 all_reduce**（推理只有前向，每层 2 次——vLLM 那篇的数字）。
...
这是第七篇编译器与本篇通信的交汇点：图优化的对象不再只是算子，也包括通信。vLLM 那篇 §2.3.2 讲的同一件事是推理侧的版本。
...
PP 的代价不是通信而是气泡和负载不均。vLLM 那篇 §1.3 讨论了推理侧 PP 的另一组问题：没有反向所以没有 1F1B 的问题，但请求长度动态变化，气泡更难消除；KV Cache 按 stage 分布。
...
推理侧的 CP 有不同的形态（Prefill 用序列切分、Decode 切 KV Cache），见 vLLM 那篇 §1.5。
...
路由细节、capacity factor 的取舍、grouped GEMM、token 重排、vLLM 中的实现，见 vLLM 那篇 §1.4，那里讲得很完整，本文不重复。

### 7. 组合与选择

#### 7.1 训练 vs 推理：同一组策略，不同的重心

vLLM 那篇的组织是"遇到什么问题、用什么策略"。本文的主线放在训练上，两者的差别集中在几点：
...
与 vLLM 那篇 §1.6 的推理版对照：推理的第 4 步不存在（参数复制是免费的），第 6 步是"DP 多实例"。
...
多维并行的 mesh 布局必须与拓扑对齐（第三章 §7.3）。vLLM 那篇 §2.4 给出了一套"确认拓扑 → 确认 NCCL 识别 → nccl-tests 区分通信库与应用问题 → 按消息规模定方向"的排查流程，训练同样适用。
