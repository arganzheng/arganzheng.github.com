...
```text
第二章   浮点数的一般形式              三段位与四类编码、八种格式的位布局、一个直观的比较
第三章   指数位与尾数位的取舍          固定 16 位的两种分法、训练中的数跨多少个数量级、为什么深度学习选范围
第四章   数值在哪些地方丢失            大数吃小数、长求和的误差积累、softmax 的指数溢出、方差计算中的相消
第五章   累加精度                      k = 4096 的点积在 BF16 中累加会怎样、FP8 Tensor Core 为什么用 FP32 累加器
第六章   混合精度训练                  三要素、master weights 为什么不能省、loss scaling、BF16 为什么不需要、训练状态的字节数
第七章   FP8 训练                      E4M3 与 E5M2 的分工、per-tensor 与 delayed scaling、DeepSeek-V3 的分块量化
第八章   推理中的数值                  attention logit 的增长与 QK-norm、RMSNorm 的 ε、两个 kernel 的数值差异、随机性与可复现
第九章   实践                          逐位构造各格式、模拟 BF16 权重更新被吃掉、GEMM 误差随 k 的增长、llm_cost.py 的 dtype 表
第十章   本文小结
```
